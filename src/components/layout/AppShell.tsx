        ...
        case 'n':
          if (!inInput) {
            ...
            window.dispatchEvent(
              new CustomEvent('cmdbar:open', { detail: { input: '> new note ' } }),
            );
          }
          break;
      }
  ...
  const renderMainContent = () => {
    ...
    if (activeTab) {
      ...
      if (activeTab.type === 'image')
        return /\.svg$/i.test(activeTab.relativePath ?? '') ? (
      ...
      if (activeTab.type === 'pdf')
        return <PdfView key={activeDocumentKey} relativePath={activeTab.relativePath} />;
      if (activeTab.type === 'logic')
        return <LogicDiagramView key={activeDocumentKey} relativePath={activeTab.relativePath} />;
          ...
          <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
            ...
            <div className="relative flex-1 overflow-hidden">
              <EditorErrorBoundary key={activeTabPath ?? activeView}>
                <div
                  key={`${activeView}:${activeDocumentKey}`}
                  className="h-full min-h-0 app-view-enter"
                >
                  {renderMainContent()}
                </div>
              </EditorErrorBoundary>
...
function EmptyEditor() {
  ...
  const hints: Record<string, { icon: React.ReactNode; title: string; hint: string }> = {
    ...
    editor: {
      ...
      title: 'No file open',
      hint: 'Select a file from the sidebar or press Ctrl+K to search.',
    },
