...
Nothing here proves a preview layout _inflates_. The unit test only checks that
...
generated children for … with N children`. Lists are grouped through
  ...
  _how much data the tester has_, so validate on a profile with many items, not
  a clean one.
