...
| Digital ink and annotation | Phases 0-2 complete, device gate open | The `.ink` contract, the shared ink domain, and the full vault lifecycle are done: New Drawing with paper presets, tabs, routing, save/conflict handling, hosted classification, and reference rewrites. Drawing itself is Phase 3. Run `tools/ink-input-probe.html` on real pens and tablets to close the Phase 0 device gate — Phase 3 depends on it. | [Digital Ink And Annotation Plan](./digital-ink-and-annotation-plan.md), [Phase 0 Contract](./digital-ink-phase0-contract.md) |
