...
describe('advanced ink tools', () => {
  it('snaps endpoints to a bounded document grid', () => {
    expect(snapInkPoint({ x: 740, y: 1_170 }, { enabled: true, spacing: 768 })).toEqual({
      x: 768,
      y: 1_536,
    });
    expect(snapInkPoint({ x: 740, y: 1_170 }, { enabled: false, spacing: 768 })).toEqual({
      x: 740,
      y: 1_170,
    });
  });
  ...
  it('builds deterministic geometry and honours the uniform modifier', () => {
    expect(shapePoints('rectangle', { x: 0, y: 0 }, { x: 100, y: 40 }, true)).toEqual([
  ...
  it('recognizes closed rectangles and ellipses without mutating the source stroke', () => {
    ...
    const ellipse = stroke(
      Array.from({ length: 25 }, (_, index) => {
        ...
        return {
          x: Math.round(100 + Math.cos(angle) * 100),
          y: Math.round(80 + Math.sin(angle) * 80),
        };
      }),
    );
    expect(recognizeInkShape(rectangle)).toMatchObject({
      shape: 'rectangle',
      sourceStrokeId: rectangle.id,
    });
    expect(recognizeInkShape(ellipse)).toMatchObject({
      shape: 'ellipse',
      sourceStrokeId: ellipse.id,
    });
