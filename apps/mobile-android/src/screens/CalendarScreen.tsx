...
import { reconcileProfileCalendarReminders } from '../../../../src/lib/calendarReminderScheduler';
...
import { mobileCalendarProfileId, resolveMobileCalendarMirrorConflict } from '../lib/calendarSync';
...
function calendarOrigin(calendar: CalendarDefinition): string {
  ...
  if (calendar.location.kind === 'hosted') {
    try {
      return new URL(calendar.location.serverUrl).host;
    } catch {
      return calendar.location.serverUrl;
    }
  }
...
export function CalendarScreen({ prefs }: { prefs: ThemePrefs }) {
  ...
  const [transition, setTransition] = useState<{ direction: -1 | 1; sequence: number }>({
    direction: 1,
    sequence: 0,
  });
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      let definitions = (await listProfileCalendars(profileId)).filter(
      ...
      const loadedItems = await listProfileCalendarItems(
        profileId,
        range.from,
        range.to,
        2_000,
      ...
      const activeIds = new Set(
        definitions.filter((calendar) => !calendar.archived).map((calendar) => calendar.id),
      );
  ...
  const currentUserIds = useMemo(
    () =>
      new Set(Object.values(statuses).flatMap((status) => (status.user ? [status.user.id] : []))),
    [statuses],
  );
  ...
  const visibleItems = useMemo(
    () =>
      projectedItems.filter(
        (item) =>
          visibleIds.has(item.calendarId) &&
          (prefs.calendarShowDeclined ||
            !item.attendees.some(
              (attendee) =>
                attendee.kind === 'collabUser' &&
                currentUserIds.has(attendee.userId) &&
  ...
  const openAttachment = async (attachment: CalendarAttachment) => {
    ...
    if (!serverUrl || !vaultId)
      throw new Error('Open the referenced vault to view this attachment.');
    ...
    const file = current.files.find(
      (entry) => entry.id === attachment.fileId || entry.relativePath === attachment.fileId,
    );
  ...
  const changeView = (next: MobileCalendarView) => {
    if (next === view) return;
    animate(
      VIEW_OPTIONS.findIndex((option) => option.value === next) >
        VIEW_OPTIONS.findIndex((option) => option.value === view)
        ? 1
        : -1,
    );
    setView(next);
  ...
  useEffect(() => {
    const openDestination = (event: Event) => {
      ...
      const targetDate =
        destination.kind === 'calendar-today' ? dateKey(new Date()) : destination.date;
  ...
  useEffect(() => {
    ...
    const item =
      projectedItems.find((candidate) => candidate.id === pendingNotificationItemId) ??
  ...
  return (
    <div className="screen mobile-calendar-screen">
      <header className="screen-header mobile-calendar-main-head">
        ...
        <div className="screen-header-actions">
          ...
          <button
            type="button"
            className="icon-button calendar-invitation-button"
          ...
          <button
            ...
            disabled={syncing}
          >
            {syncing ? <Spinner size={17} /> : <RefreshCw size={17} />}
          </button>
          <button
            type="button"
      ...
      {Object.values(mirrorProgress).some(
        (entry) => entry.phase === 'checking' || entry.phase === 'applying',
      ) ? (
        <Banner tone="info">
          <span>
            {Object.values(mirrorProgress).find(
              (entry) => entry.phase === 'applying' || entry.phase === 'checking',
            )?.detail ?? 'Updating calendar mirrors'}
          </span>
        </Banner>
      ) : null}
      {conflicts.length > 0 ? (
        <button
          type="button"
          className="calendar-conflict-banner"
          onClick={() => setConflictsOpen(true)}
        >
          <span>
            {conflicts.length} calendar change{conflicts.length === 1 ? '' : 's'} need attention.
          </span>
          <span>Review</span>
        </button>
      ) : null}
      {mirrorStatuses.some(
        (status) =>
          status.state === 'waiting' || status.state === 'conflict' || status.state === 'error',
      ) ? (
        <button
          type="button"
          className="calendar-conflict-banner"
        ...
        >
          <span>
            {mirrorConflicts.length > 0
              ? `${mirrorConflicts.length} mirrored item${mirrorConflicts.length === 1 ? '' : 's'} need attention.`
              : mirrorStatuses.some((status) => status.state === 'error')
                ? 'A calendar mirror could not be updated.'
                : 'A calendar mirror is waiting for a server connection.'}
          </span>
          <span>Manage</span>
        </button>
      ) : null}
        ...
        {calendars
          ...
          .map((calendar) => (
            ...
            >
              ...
              <small>{calendarOrigin(calendar)}</small>
            </button>
          ))}
        <button
          type="button"
      ...
      >
        {busy ? (
          <div className="calendar-mobile-loading">
            <Spinner />
        ...
        {!busy && view === 'month' ? (
          <MonthView
            ...
            hideWeekends={prefs.calendarHideWeekends}
            onStep={moveMonth}
            onSelect={openDay}
            onOpen={(item) => setEditor({ kind: item.kind, item })}
          />
        ) : null}
        {!busy && view === 'agenda' ? (
          <AgendaView
        ...
        {!busy && view === 'three-day' ? (
          <ThreeDayView
            ...
            calendarById={calendarById}
            timeFormat={prefs.calendarTimeFormat}
            workingHoursStart={prefs.calendarWorkingHoursStart}
            onStep={moveThreeDay}
            onOpenDay={openDay}
            onAddAt={(date, initialTime) => {
              setSelectedDate(date);
              setAnchor(parseDate(date));
              setEditor({ kind: 'event', initialTime });
            }}
            onOpen={(item) => setEditor({ kind: item.kind, item })}
          />
        ) : null}
        {!busy && view === 'day' ? (
          <DayView
            ...
            workingHoursStart={prefs.calendarWorkingHoursStart}
            onStep={moveDay}
            onAddAt={(initialTime) => setEditor({ kind: 'event', initialTime })}
            onOpen={(item) => setEditor({ kind: item.kind, item })}
          />
        ) : null}
        {!busy && view === 'tasks' ? (
        ...
        {!busy && visibleItems.length === 0 && calendars.length === 0 ? (
          <EmptyState
            icon={<CalendarDays size={28} />}
            title="No calendars"
            message="Connect a server or create a local calendar."
          />
        ) : null}
      </div>
      ...
      {editor ? (
        <MobileItemEditor
          ...
          calendars={calendars.filter(
            (calendar) =>
              !calendar.archived && (!calendar.readOnly || calendar.id === editor.item?.calendarId),
          )}
          ...
          onOpenAttachment={(attachment) =>
            void openAttachment(attachment).catch((reason) => setError(String(reason)))
          }
          onSaved={async () => {
            setEditor(null);
            await refresh();
        ...
        <CalendarManager
          ...
          onChanged={async (calendarId, visible) => {
            if (calendarId && visible != null) {
              setVisibleIds((current) => {
                const next = new Set(current);
                if (visible) next.add(calendarId);
      ...
      {invitationsOpen ? (
        <MobileInvitations
          origins={Object.values(statuses).flatMap((status) =>
...
function MobileInvitations({
  ...
  useEffect(() => {
    ...
    void Promise.all(
      origins.map(async ({ serverUrl }) => {
        const entries = await hostedCalendarRequest<HostedInvitation[]>(
          serverUrl,
          'GET',
  ...
  return (
    <div className="sheet-backdrop" onClick={onClose}>
      <div
        className="sheet calendar-relations-sheet"
        role="dialog"
        aria-label="Calendar invitations"
      ...
      >
        ...
        {!busy && invitations.length === 0 ? (
          <EmptyState
            ...
            message="Meeting invitations from your connected servers appear here."
          />
        ) : null}
        <div className="calendar-relation-list">
          {invitations.map((invitation) => (
            <article key={`${invitation.serverUrl}:${invitation.id}`}>
              <CalendarDays size={17} />
              <div>
                <strong>{invitation.item.title}</strong>
                <small>
                  {calendarServerLabel(invitation.serverUrl)} ·{' '}
                  {invitation.response.replace('-', ' ')}
                </small>
              </div>
              <div>
                {(['accepted', 'tentative', 'declined'] as CalendarAttendanceResponse[]).map(
                  (response) => (
                    <button
                      key={response}
                      type="button"
                      className={invitation.response === response ? 'active' : ''}
                      disabled={busy}
...
function CalendarConflictRecovery({
  conflicts,
  profileId,
  onClose,
  onChanged,
  ...
  return (
    <div className="sheet-backdrop" onClick={onClose}>
      <div
        className="sheet calendar-relations-sheet"
        role="dialog"
        aria-label="Calendar sync issues"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="sheet-handle" />
        <div className="sheet-head">
        ...
        <div className="calendar-relation-list">
          {conflicts.map((conflict) => {
            ...
            const label =
              mutation.type === 'upsertItem'
                ? mutation.item.title
                : mutation.type === 'deleteItem'
                  ? 'Deleted calendar item'
                  : mutation.type === 'createCalendar' || mutation.type === 'updateCalendar'
                    ? mutation.calendar.name
                    : 'Calendar';
            return (
              <article key={id}>
                <RefreshCw size={17} />
                ...
                <div>
                  <button type="button" disabled={!!busyId} onClick={() => void act(id, 'retry')}>
                    Retry
                  </button>
                  <button
                    type="button"
...
function CalendarManager({
  ...
  const mirrorEligible = calendars.filter(
    (calendar) =>
      !calendar.archived &&
      !calendar.deletedAt &&
      !calendar.readOnly &&
  ...
  useEffect(() => {
    ...
    void listProfileCalendarMirrorGroups(profileId)
      .then((groups) => {
        if (active) setMirrorGroups(groups);
      })
      .catch((reason) => {
        if (active) setError(String(reason));
      });
    ...
    const local = [
      {
        ...
        location: { kind: 'local', profileId } as CalendarLocation,
      },
    ];
  ...
  const startEdit = (calendar: CalendarDefinition) =>
    setDraft({
      calendar,
      name: calendar.name,
  ...
  const persistUpdate = async (
    existing: CalendarDefinition,
    changes: Partial<CalendarDefinition>,
  ) => {
  ...
  const toggleMirror = async (group: CalendarMirrorGroup, enabled: boolean) => {
    ...
    try {
      ...
      setMirrorGroups((current) =>
        current.map((entry) => (entry.id === group.id ? updated : entry)),
      );
    } catch (reason) {
      setError(String(reason));
    } finally {
  ...
  const selectedMirrorLocations = new Set(
    mirrorCalendarIds.flatMap((calendarId) => {
      const calendar = mirrorEligible.find((entry) => entry.id === calendarId);
  ...
  return (
    <div className="sheet-backdrop" onClick={onClose}>
      <div
        className="sheet calendar-manager-sheet"
        role="dialog"
        aria-label="Manage calendars"
        onClick={(event) => event.stopPropagation()}
      >
        ...
        <div className="sheet-head">
          ...
          <button
            type="button"
            className="icon-button"
            aria-label="Close calendar manager"
            onClick={onClose}
          >
            <X size={18} />
          </button>
        ...
        <button
          type="button"
          className="calendar-manager-add"
          onClick={startCreate}
          disabled={saving}
        >
          <Plus size={16} />
        ...
        <div className="calendar-manager-list">
          {active.map((calendar) => (
            <CalendarManagerRow
              ...
              saving={saving}
              onEdit={startEdit}
              onImport={() => void importCalendar(calendar)}
              onExport={() => void exportCalendar(calendar)}
        ...
        <section className="calendar-mirror-manager" aria-labelledby="mobile-calendar-mirrors">
          <div className="calendar-mirror-manager-head">
            <h3 id="mobile-calendar-mirrors">Mirrors</h3>
            <button
              type="button"
              className="icon-button"
              aria-label="Add calendar mirror"
              disabled={saving}
              onClick={() => setAddingMirror((current) => !current)}
            >
              <Link2 size={16} />
            </button>
          </div>
          {mirrorGroups.map((group) => {
            ...
            return (
              <div key={group.id} className="calendar-mirror-manager-group">
                <div className="calendar-mirror-manager-row">
                  <button
                    type="button"
                    className={group.enabled ? 'active' : ''}
                    aria-pressed={group.enabled}
                    disabled={saving}
                    onClick={() => void toggleMirror(group, !group.enabled)}
                  >
                    <Link2 size={15} />
                  </button>
                  <div>
                  ...
                  {status?.state === 'waiting' ? (
                    <CloudOff size={15} className="calendar-mirror-waiting" />
                  ) : (
                    <span />
                  )}
                  <button
                    ...
                    disabled={saving}
                    onClick={() => void deleteMirror(group.id)}
                  >
                    <Trash2 size={15} />
                  </button>
                </div>
                {conflicts.map((conflict) => (
                  <div key={conflict.id} className="calendar-mirror-conflict">
                    <strong>Choose the version to keep</strong>
                    {conflict.versions.map((version) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => void resolveMirror(group, conflict, version.memberId)}
                        >
                          <span style={{ background: calendar?.color }} />
                          <span>
                            <strong>
          ...
          {mirrorGroups.length === 0 && !addingMirror ? (
            <p className="calendar-manager-origin">No mirror groups yet.</p>
          ) : null}
          {addingMirror ? (
            <div className="calendar-manager-editor">
              ...
              <fieldset className="calendar-mirror-calendar-list">
                <legend>Calendars</legend>
                {mirrorEligible.map((calendar) => {
              ...
              <div className="form-actions">
                ...
                <button
                  ...
                  disabled={saving || !mirrorName.trim() || mirrorCalendarIds.length < 2}
                  onClick={() => void saveMirror()}
                >
                  {saving ? <Spinner /> : null}Create mirror
                </button>
              </div>
            </div>
          ) : null}
        ...
        {draft ? (
          <div className="calendar-manager-editor">
            ...
            {!draft.calendar ? (
              <fieldset className="calendar-manager-locations">
                <legend>Location</legend>
                {locations.map((option) => {
                  const selected =
                    calendarLocationKey(draft.location) === calendarLocationKey(option.location);
                  return (
                    <button
                      key={option.key}
                      type="button"
                      ...
                      onClick={() =>
                        setDraft((current) =>
                          current ? { ...current, location: option.location } : current,
                        )
                      }
                    >
                      {option.location.kind === 'local' ? 'Device' : 'Server'}
              ...
              <p className="calendar-manager-origin">Stored in {calendarOrigin(draft.calendar)}</p>
            )}
            <fieldset className="calendar-manager-colors">
...
function CalendarManagerRow({
  ...
  return (
    <div className="calendar-manager-row">
      <span style={{ background: calendar.color }} />
      <div>
        <strong>{calendar.name}</strong>
        <small>
      ...
      <button
        type="button"
        className="icon-button"
        aria-label={`Import into ${calendar.name}`}
      ...
      <button
        type="button"
        className="icon-button"
        aria-label={`${calendar.archived ? 'Restore' : 'Archive'} ${calendar.name}`}
        disabled={saving || calendar.readOnly}
        onClick={onArchive}
      >
        {calendar.archived ? <ArchiveRestore size={15} /> : <Archive size={15} />}
      </button>
    </div>
  );
}
...
function MonthView({
  ...
  return (
    <section className="calendar-mobile-month">
      ...
      <div className="calendar-mobile-weekdays" style={columnStyle}>
        {weekdayDates.map((day) => (
          <span key={day.getDay()}>{day.toLocaleDateString(undefined, { weekday: 'narrow' })}</span>
        ))}
      </div>
      <div className="calendar-mobile-grid" style={columnStyle}>
          ...
          return (
            ...
            >
              ...
              <div className="calendar-mobile-cell-items">
                ...
                {dayItems.length > 3 ? (
                  <span className="calendar-mobile-more">+{dayItems.length - 3}</span>
                ) : null}
              </div>
            </div>
          );
...
function AgendaView({
  ...
  const days = Array.from({ length: dayCount }, (_, index) =>
    dateKey(new Date(anchor.getFullYear(), anchor.getMonth(), index + 1)),
  );
  const populated = days
    .map((day) => ({ day, items: items.filter((item) => itemOccursOn(item, day)) }))
    ...
    <div className="calendar-mobile-agenda">
      {populated.length === 0 ? (
        <EmptyState
          ...
          message="This month has no visible calendar items."
        />
      ) : (
        populated.map((group) => (
          <section key={group.day}>
            <h2>
              {parseDate(group.day).toLocaleDateString(undefined, { weekday: 'long' })},{' '}
              {formatCalendarDate(parseDate(group.day), dateFormat)}
            </h2>
            <ItemList
              items={group.items}
              calendarById={calendarById}
...
function timelineItemRangeLabel(
  item: CalendarItem,
  format: ThemePrefs['calendarTimeFormat'],
): string {
  if (item.kind === 'birthday') return 'Birthday';
...
function TimelineColumn({
  ...
  return (
    <div className="calendar-mobile-time-column">
      {Array.from({ length: 48 }, (_, index) => {
        ...
        return (
          <button
            ...
            style={{
              top: (minute / 60) * MOBILE_DAY_HOUR_HEIGHT,
              height: MOBILE_DAY_HOUR_HEIGHT / 2,
            }}
      ...
      {Array.from({ length: 24 }, (_, hour) => (
        <div
          key={hour}
          className="calendar-mobile-hour-line"
          style={{ top: hour * MOBILE_DAY_HOUR_HEIGHT }}
        />
      ))}
      {today ? (
        <div
          className="calendar-mobile-now"
          style={{ top: (nowMinute / 60) * MOBILE_DAY_HOUR_HEIGHT }}
        >
          <i />
        </div>
      ) : null}
            ...
            style={
              {
                ...
                height: Math.max(
                  28,
                  ((entry.endMinute - entry.startMinute) / 60) * MOBILE_DAY_HOUR_HEIGHT,
                ),
...
function TimelineGutter({ timeFormat }: { timeFormat: ThemePrefs['calendarTimeFormat'] }) {
  return (
    <div className="calendar-mobile-time-gutter">
      {Array.from({ length: 24 }, (_, hour) => (
        <span key={hour} style={{ top: hour * MOBILE_DAY_HOUR_HEIGHT }}>
          {calendarClockLabel(new Date(2026, 0, 1, hour), timeFormat)}
        </span>
      ))}
    </div>
  );
...
function ThreeDayView({
  ...
  const dayGroups = days.map((date) => {
    ...
    const timedIds = new Set(
      layoutCalendarTimedItems(dayItems, date).map((entry) => entry.item.id),
    );
    return {
      date,
      items: dayItems,
      allDayItems: dayItems.filter((item) => !timedIds.has(item.id)),
    };
  });
  ...
  return (
    <section ref={rootRef} className="calendar-mobile-three-day">
        ...
        {days.map((date) => {
          ...
          return (
            <button
              key={date}
              type="button"
              aria-label={`Open ${parsed.toLocaleDateString()} day view`}
      ...
      {hasAllDayItems ? (
        <div className="calendar-mobile-three-day-all-day">
          <span>All day</span>
          {dayGroups.map((group) => (
            <div key={group.date}>
              {group.allDayItems.slice(0, 2).map((item) => {
                const color = calendarById.get(item.calendarId)?.color ?? '#a78bfa';
                return (
                  <button
                    key={item.id}
              ...
              {group.allDayItems.length > 2 ? <small>+{group.allDayItems.length - 2}</small> : null}
            </div>
          ))}
        </div>
      ) : null}
      <div
        className="calendar-mobile-three-day-timeline"
        style={{ height: MOBILE_DAY_HOUR_HEIGHT * 24 }}
      >
        ...
        {dayGroups.map((group) => (
          <TimelineColumn
            ...
            calendarById={calendarById}
            timeFormat={timeFormat}
            onAddAt={(time) => onAddAt(group.date, time)}
            onOpen={onOpen}
          />
        ))}
      </div>
...
function DayView({
  ...
  return (
    <section ref={rootRef} className={`calendar-mobile-day ${today ? 'today' : ''}`}>
      <div className="mobile-calendar-toolbar calendar-mobile-day-toolbar">
        <button
          type="button"
          className="icon-button"
        ...
        <strong aria-current={today ? 'date' : undefined}>
          {formatCalendarDate(parseDate(date), dateFormat)}
        </strong>
        <button
          type="button"
          className="icon-button"
      ...
      {allDayItems.length > 0 ? (
        <div className="calendar-mobile-all-day">
          <span>All day</span>
          <div>
        ...
        return (
          ...
          >
            ...
            <span>
              ...
              <small>
                {itemTimeLabel(item, timeFormat)} · {calendar?.name ?? 'Calendar'} · {item.kind}
              </small>
            </span>
          </button>
        );
...
function MobileItemEditor({
  ...
  request: { kind: CalendarItemKind; item?: CalendarItem; initialTime?: string };
  date: string;
  calendars: CalendarDefinition[];
  ...
  onOpenAttachment: (attachment: CalendarAttachment) => void;
}) {
  // Back closes this sheet instead of prompting to quit the app.
  useBackDismiss(true, onClose);
  ...
  const [startDate, setStartDate] = useState(
    request.item?.kind === 'task'
      ? request.item.start?.kind === 'date'
        ? request.item.start.date
        : (request.item.start?.dateTime.slice(0, 10) ?? '')
      : request.item
        ? itemDate(request.item)
        : date,
  );
  const [endDate, setEndDate] = useState(
    request.item?.kind === 'task'
      ? request.item.due?.kind === 'date'
        ? request.item.due.date
        : (request.item.due?.dateTime.slice(0, 10) ?? '')
      : request.item?.kind === 'event' && request.item.end.kind === 'date'
        ? addDays(request.item.end.date, -1)
        : request.item
          ? itemDate(request.item)
          : date,
  );
  const existingStart =
    request.item?.kind === 'event'
      ? request.item.start
      : request.item?.kind === 'task'
        ? (request.item.start ?? request.item.due)
        : undefined;
  const existingEnd =
    request.item?.kind === 'event'
      ? request.item.end
      : request.item?.kind === 'task'
        ? (request.item.due ?? request.item.start)
        : undefined;
  ...
  const [startTime, setStartTime] = useState(
    existingStart?.kind === 'dateTime'
      ? new Date(existingStart.dateTime).toTimeString().slice(0, 5)
      : initialStartTime,
  );
  const [endTime, setEndTime] = useState(
    existingEnd?.kind === 'dateTime'
      ? new Date(existingEnd.dateTime).toTimeString().slice(0, 5)
      : addMinutes(initialStartTime, prefs.calendarDefaultDurationMinutes),
  );
  ...
  const [recurrence, setRecurrence] = useState(
    knownRule ? initialRule : initialRule ? 'CUSTOM' : '',
  );
  const [customRecurrence, setCustomRecurrence] = useState(knownRule ? '' : initialRule);
  const [reminders, setReminders] = useState<number[]>(() => {
    const relative =
      request.item?.reminders.flatMap((entry) =>
        entry.kind === 'relative' ? [entry.minutesBefore] : [],
      ) ?? [];
    return request.item
      ? relative
      : prefs.calendarDefaultReminderMinutes === null
        ? []
        : [prefs.calendarDefaultReminderMinutes];
  });
  const [customReminder, setCustomReminder] = useState('120');
  const [taskStatus, setTaskStatus] = useState<CalendarTaskStatus>(
    request.item?.kind === 'task' ? request.item.status : 'needs-action',
  );
  const [location, setLocation] = useState(
    request.item?.kind === 'event' ? (request.item.location?.label ?? '') : '',
  );
  ...
  const persistItem = async (item: CalendarItem) => {
    ...
    if (calendar.location.kind === 'local')
      await acknowledgeProfileCalendarOperations(profileId, [operation.clientOperationId]);
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      const base = {
        id,
        uid: request.item?.uid ?? `${id}@collab.mobile`,
        calendarId,
        title: title.trim(),
      ...
      const timeZone =
        calendar.defaultTimeZone || Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC';
      ...
      const item =
        kind === 'event'
          ? normalizeCalendarItem({
              ...
              kind,
              start: startValue!,
              end: allDay ? { kind: 'date', date: addDays(endDate, 1) } : endValue!,
              location: location.trim()
                ? { label: location.trim(), address: location.trim() }
                : undefined,
          ...
          : kind === 'task'
            ? normalizeCalendarItem({
                ...
                due: endValue,
                status: taskStatus,
                priority: request.item?.kind === 'task' ? request.item.priority : undefined,
                completedAt:
                  taskStatus === 'completed'
                    ? ((request.item?.kind === 'task' ? request.item.completedAt : undefined) ??
                      now)
                    : undefined,
              })
            : normalizeCalendarItem({
      ...
      if (!kanbanBound && request.item?.recurrenceId && request.item.recurrenceSeriesId) {
        ...
        if (!master?.recurrence)
          throw new Error('The recurring series is not available in the local cache.');
        const recurrenceKey = calendarTimeValueKey(request.item.recurrenceId);
        const existingException = sourceItems.find(
          (entry) =>
            entry.uid === master.uid &&
    ...
    try {
      ...
      if (target.recurrenceId && target.recurrenceSeriesId) {
        ...
        if (editScope === 'occurrence') {
          ...
          if (
            !exdates.some(
              (value) => calendarTimeValueKey(value) === calendarTimeValueKey(target.recurrenceId!),
            )
          )
            exdates.push(target.recurrenceId);
        ...
        if (editScope === 'following') {
          const recurrence = splitRecurrence(
            master.recurrence,
            target.recurrenceId,
            priorOccurrenceCount(master, target),
          ).previous;
          if (recurrence) {
      ...
      await deleteProfileCalendarItem(
        profileId,
        target.calendarId,
        target.id,
        deletedAt,
  ...
  return (
    <div className="sheet-backdrop" onClick={onClose}>
      <div
        className="sheet calendar-item-sheet"
        role="dialog"
        aria-label={request.item ? 'Edit calendar item' : 'New calendar item'}
        onClick={(event) => event.stopPropagation()}
      >
        <div className="sheet-handle" />
        <div className="sheet-head">
          <strong>{request.item ? 'Edit item' : 'New item'}</strong>
        ...
        {kanbanBound ? (
          <Banner tone="info">
            Dates, completion, and supported recurrence write back to Kanban. Other details remain
            source-owned.
          </Banner>
        ) : null}
        <div className="calendar-kind-picker" role="group" aria-label="Item type">
          {(['event', 'task', 'birthday'] as CalendarItemKind[]).map((value) => (
            <button
              key={value}
              type="button"
              className={kind === value ? 'active' : ''}
              disabled={!!request.item}
        ...
        <label className="form-field">
          ...
          <input
            ...
            autoFocus
            disabled={kanbanBound}
          />
        </label>
        <fieldset className="calendar-editor-calendars" disabled={kanbanBound}>
          <legend>Calendar</legend>
          {calendars.map((calendar) => (
            <button
              key={calendar.id}
              type="button"
              className={calendarId === calendar.id ? 'active' : ''}
        ...
        {kind !== 'birthday' ? (
          <button
            ...
            role="switch"
            aria-checked={allDay}
            className="calendar-all-day-toggle"
            disabled={kanbanBound}
        ...
        <label className="form-field">
          <span>{kind === 'birthday' ? 'Birthday' : 'Start'}</span>
          <DateField
          ...
          />
          {kind !== 'birthday' && !allDay && startDate ? (
            <TimeField
              label="Start time"
              format={prefs.calendarTimeFormat}
              value={startTime}
              onChange={(value) => {
                setStartTime(value);
        ...
        {kind !== 'birthday' ? (
          <label className="form-field">
            <span>{kind === 'task' ? 'Deadline' : 'End'}</span>
            <DateField
        ...
        {kind !== 'birthday' && !allDay && endDate ? (
          <div className="form-field">
            <span>{kind === 'task' ? 'Deadline time' : 'End time'}</span>
            <TimeField
              label={kind === 'task' ? 'Deadline time' : 'End time'}
              format={prefs.calendarTimeFormat}
              value={endTime}
              onChange={setEndTime}
            />
        ...
        {kind === 'task' ? (
          <fieldset className="calendar-editor-options">
            <legend>Status</legend>
            {(['needs-action', 'in-progress', 'completed', 'cancelled'] as CalendarTaskStatus[])
              .filter((value) => !kanbanBound || value === 'needs-action' || value === 'completed')
              .map((value) => (
                <button
                  key={value}
                  type="button"
                  className={taskStatus === value ? 'active' : ''}
              ...
              <fieldset className="calendar-editor-options">
                ...
                {(
                  ...
                  <button
                    key={value}
                    type="button"
                    className={editScope === value ? 'active' : ''}
        ...
        {kind === 'event' ? (
          <label className="form-field">
            <span>Location</span>
            <div className="calendar-location-field">
              ...
              <button
                ...
                onClick={() =>
                  void openExternalUrl(`geo:0,0?q=${encodeURIComponent(location.trim())}`).catch(
        ...
        {!kanbanBound ? (
          <fieldset className="calendar-editor-options">
            ...
            {REMINDER_OPTIONS.map((option) => {
              const active =
                option.value < 0 ? reminders.length === 0 : reminders.includes(option.value);
              return (
                <button
                  key={option.value}
                  type="button"
            ...
            <div className="calendar-custom-reminder">
              <input
              ...
              <button
                type="button"
                disabled={!Number(customReminder) || reminders.includes(Number(customReminder))}
                onClick={() =>
                  setReminders((current) =>
                    [...current, Number(customReminder)].sort((left, right) => left - right),
                  )
                }
              >
                Add
        ...
        <div className="form-actions calendar-item-actions">
          {request.item && !kanbanBound ? (
            <button
              type="button"
          ...
          <button
            ...
            disabled={
              ...
              !title.trim() ||
              !calendarId ||
              (recurrence === 'CUSTOM' && !customRecurrence.trim())
            }
            onClick={() => void save()}
          >
            {saving ? <Spinner /> : null}
            {request.item ? 'Save' : 'Create'}
          </button>
        </div>
      </div>
    </div>
...
function MobileAttendeeEditor({
  ...
  return (
    <section className="calendar-editor-relations">
      <h3>
        <Users size={15} />
        People
      </h3>
      {attendees.map((attendee) => (
        <div className="calendar-editor-relation" key={attendee.id}>
          <Users size={15} />
          <span>
            <strong>
              {attendee.displayName ??
                (attendee.kind === 'email' ? attendee.email : attendee.userId)}
            </strong>
            <small>
              {attendee.role} · {attendee.response.replace('-', ' ')}
            </small>
          </span>
          <button
            type="button"
      ...
      <label className="calendar-relation-search">
        <UserPlus size={15} />
        <input
      ...
      {query.trim().length >= 2 && !busy ? (
        <div className="calendar-relation-results">
          {results.map((entry) => (
            <button key={entry.userId} type="button" onClick={() => add(entry)}>
              <span>{entry.displayName || entry.username}</span>
              <small>@{entry.username}</small>
            </button>
          ))}
          {results.length === 0 ? <small>No people found</small> : null}
        </div>
      ) : null}
    </section>
  );
...
function MobileAttachmentEditor({
  calendar,
  attachments,
  onChange,
  onOpen,
  ...
  const [boards, setBoards] = useState<
    Array<{ fileId: string; fileName: string; cardId: string; cardTitle: string }>
  >([]);
  const [url, setUrl] = useState('');
  const [busy, setBusy] = useState(false);
  const referenceOrigin = selected
    ? { serverUrl: selected.serverUrl, vaultId: selected.vault.id }
    : {};
  const attachFile = (fileId: string) => {
    const file = files.find((entry) => entry.id === fileId);
    if (
      !file ||
      attachments.some((entry) => entry.kind === 'vaultFile' && entry.fileId === file.id)
    )
      return;
    onChange([
      ...attachments,
  ...
  const loadKanbanCards = async () => {
    ...
    try {
      const entries = await Promise.all(
        files
          .filter((file) => file.documentType === 'kanban')
          .map(async (file) => {
            const document = await readHostedDocument(
              selected.serverUrl,
              selected.vault.id,
              file.id,
            );
    ...
    if (
      attachments.some(
        (attachment) =>
          ...
          attachment.fileId === entry.fileId &&
          attachment.cardId === entry.cardId,
      )
    )
      return;
    onChange([
  ...
  const upload = async () => {
    ...
    const [sourcePath] = await showMobileOpenFiles([
      'pdf',
      'txt',
      'md',
      'png',
    ...
    try {
      ...
      const uploaded = await hostedCalendarRequest<{
        id: string;
        name: string;
        mediaType?: string;
        sizeBytes?: number;
  ...
  return (
    <section className="calendar-editor-relations">
      ...
      {attachments.map((attachment) => (
        <div className="calendar-editor-relation" key={attachment.id}>
          {attachment.kind === 'kanbanTask' ? (
          ...
          <button
            type="button"
            className="calendar-attachment-open"
            disabled={attachment.kind === 'uploaded'}
      ...
      <div className="calendar-attachment-actions">
        {selected ? (
          <button type="button" onClick={() => setShowFiles((value) => !value)}>
            <Paperclip size={15} />
            Vault file
          </button>
        ) : null}
        {selected ? (
          <button type="button" disabled={busy} onClick={() => void loadKanbanCards()}>
            <SquareKanban size={15} />
            Kanban task
          </button>
        ) : null}
      ...
      {showFiles ? (
        <div className="calendar-relation-results">
          {files
            ...
            .map((file) => (
              <button key={file.id} type="button" onClick={() => attachFile(file.id)}>
                <span>{file.name}</span>
      ...
      {boards.length > 0 ? (
        <div className="calendar-relation-results">
          {boards.map((entry) => (
            <button
              key={`${entry.fileId}:${entry.cardId}`}
              type="button"
              onClick={() => attachCard(entry)}
            >
      ...
      <div className="calendar-attachment-url">
        <input
          ...
          placeholder="https://..."
        />
        <button type="button" disabled={!url.trim()} onClick={addUrl}>
          Add link
        </button>
      </div>
    </section>
