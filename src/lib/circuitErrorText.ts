export function circuitErrorText(error: unknown): string {
  ...
  switch (code) {
    case 'missingGround':
      return 'Add one ground reference before running the simulation.';
    case 'multipleGrounds':
      return 'This baseline currently requires exactly one ground symbol.';
    ...
    case 'missingWireHandle':
      return 'A wire is not attached to a valid component terminal.';
    case 'unknownWireNode':
    case 'unknownTerminal':
      return 'A wire references a component or terminal that no longer exists.';
    case 'duplicateProbeId':
      return 'Two circuit probes use the same ID. Remove and recreate one of them.';
    case 'unknownProbeNode':
      return 'A circuit probe references a component that no longer exists.';
    case 'missingProbeHandle':
      return 'A voltage probe is not attached to a component terminal.';
    case 'unknownProbeTerminal':
      return 'A voltage probe references a terminal that no longer exists.';
    case 'invalidBranchProbeTarget':
      return 'Branch-current probes cannot target the ground reference or a junction.';
    ...
    case 'singularSystem':
      return 'The circuit is floating, underconstrained, or contains conflicting ideal sources.';
    case 'invalidResistance':
      return 'Every resistor must have resistance greater than zero.';
    case 'invalidCapacitance':
      return 'Every capacitor must have capacitance greater than zero.';
    case 'invalidInductance':
      return 'Every inductor must have inductance greater than zero.';
    case 'invalidSwitchResistance':
      return 'The switch model contains an invalid resistance.';
    case 'invalidDiodeModel':
      return 'The diode model contains invalid parameters.';
    case 'invalidBipolarModel':
      return 'The transistor model contains invalid parameters.';
    ...
    case 'invalidTimeRange':
      return 'Transient duration and maximum timestep must both be greater than zero.';
    ...
    case 'resultBufferLimitExceeded':
      return 'The requested traces exceed the bounded circuit result-buffer limit.';
    case 'missingOutputs':
      return 'Add at least one voltage or current probe before running the analysis.';
    case 'duplicateOutput':
      return 'The analysis contains a duplicate output trace.';
    case 'unknownNodeOutput':
      return 'A voltage probe references an electrical node that no longer exists.';
    case 'unknownComponentOutput':
      return 'A current probe references a component that no longer exists.';
    case 'unknownWaveformSource':
      return 'A transient waveform references a source that no longer exists.';
    case 'unsupportedWaveformSource':
      return 'Transient waveforms can only target independent voltage or current sources.';
    ...
    case 'nonFiniteValue':
      return 'A component contains an invalid numerical value.';
    default:
