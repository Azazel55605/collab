...
export default function SettingsBackgroundSection() {
  ...
  useEffect(() => {
    tauriCommands.backgroundSettingsGet()
      .then(setSettings)
  ...
  if (loading) {
    return <div className="flex h-32 items-center justify-center text-muted-foreground"><Loader2 size={18} className="animate-spin" /></div>;
  }
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <OptionRow label="Run in background" description="Keep Collab available in the system tray after its window closes.">
          <ToggleSwitch
            checked={enabled}
            onToggle={() => void save({ ...settings, runInBackground: !enabled })}
          />
        </OptionRow>
        <OptionRow label="Close button" description="Choose whether closing the main window hides or quits Collab." disabled={!enabled}>
          <PillSelect
        ...
        <OptionRow label="Start at login" description="Launch Collab directly into the tray when you sign in." disabled={!enabled}>
          <ToggleSwitch
      ...
      <div>
        ...
        <OptionRow label="Sync while hidden" description="Keep eligible offline vaults and hosted calendars current.">
          <ToggleSwitch
        ...
        <OptionRow label="Schedule" disabled={!syncEnabled}>
          <Select
            ...
            onValueChange={(syncInterval) => void save({
              ...settings,
          ...
          >
            ...
            <SelectContent>
              {(Object.keys(INTERVAL_LABELS) as BackgroundSyncInterval[]).map((value) => (
                <SelectItem key={value} value={value}>{INTERVAL_LABELS[value]}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </OptionRow>
        <OptionRow label="Pause background sync" description="Keep the tray active without running scheduled synchronization." disabled={!syncEnabled}>
          <ToggleSwitch
