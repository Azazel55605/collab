...
export function assertProtectedRangesUnchanged(
  ...
  for (const source of before.worksheets) {
    ...
    for (const protectedRange of source.protectedRanges ?? []) {
      if (!target) throw new SheetDocumentError('invalid-structure', 'A protected worksheet cannot be removed.');
      ...
      if (!sourceBounds || !targetBounds) {
        throw new SheetDocumentError('invalid-structure', 'This edit would change a protected range.');
      }
      ...
      if (
        ...
        throw new SheetDocumentError('invalid-structure', 'This structural edit intersects a protected range.');
      }
      for (const rowId of sourceRows) {
        for (const columnId of sourceColumns) {
          ...
          if (JSON.stringify(source.cells[key]) !== JSON.stringify(target.cells[key])) {
            throw new SheetDocumentError('invalid-structure', 'This cell belongs to a protected range.');
          }
        }
      }
    }
...
export function protectedRangeLabel(
  worksheet: SheetWorksheet,
