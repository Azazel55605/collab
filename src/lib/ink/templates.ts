...
export function saveInkTemplate(template: InkDocumentTemplate): InkDocumentTemplate[] {
  const templates = [
    template,
    ...loadInkTemplates().filter((entry) => entry.id !== template.id),
...
export function parseInkTemplate(text: string): InkDocumentTemplate {
  ...
  if (
    ...
    payload.schemaVersion !== TEMPLATE_SCHEMA_VERSION ||
    !isTemplate(payload.template)
  ) {
    throw new Error('This file is not a supported drawing template.');
  }

  const shell = createInkDocument({
    name: payload.template.name,
    timestamp: payload.template.createdAt,
  });
