...
import { computedValuesForExport, exportWorkbookFile } from '../lib/sheet/conversion';
import {
...
import { protectSheetSelection, removeSheetProtection } from '../lib/sheet/protectedRanges';
import {
  findSheetMatches,
  nextSheetMatch,
...
export default function SheetView({ relativePath }: Props) {
  ...
  const worksheet = useMemo(() => (document ? activeWorksheetOf(document) : null), [document]);
      ...
      return [
        {
          ...
          active,
          ranges: ranges.length > 0 ? ranges : active ? [{ anchor: active, focus: active }] : [],
        },
      ];
    });
  }, [livePeers, worksheet]);
  ...
  const handleFormulaReferenceCommit = useCallback(
  ...
  const activeStyle = useMemo(
    () =>
      document && worksheet ? resolveCellStyle(document.styles, worksheet, selection.active) : {},
  ...
  const activeTableColors = useMemo(
    () =>
      document && worksheet && activeTable && activeTableColumn
        ? uniqueTableColumnColors(document, worksheet, activeTable, activeTableColumn.columnId)
  ...
  const writeClipboardEvent = useCallback(
    ...
    try {
      if (navigator.clipboard?.write && typeof ClipboardItem !== 'undefined') {
        await navigator.clipboard.write([
          new ClipboardItem({
            [SHEET_CLIPBOARD_MIME]: new Blob([JSON.stringify(payload)], {
              type: SHEET_CLIPBOARD_MIME,
            }),
  ...
  const applyClipboardPayload = useCallback(
    (payload: SheetClipboardPayload, mode: SheetPasteMode) => {
      mutate((current, id) =>
        pasteSheetClipboardPayload(current, id, selection.active, payload, mode),
      );
    },
        ...
        if (navigator.clipboard?.read) {
          ...
          for (const item of items) {
            if (item.types.includes(SHEET_CLIPBOARD_MIME)) {
              payload = parseSheetClipboardPayload(
                await (await item.getType(SHEET_CLIPBOARD_MIME)).text(),
              );
              if (payload) break;
            }
          }
  ...
  const handlePasteEvent = useCallback(
    (event: ReactClipboardEvent<HTMLDivElement>) => {
      if (session.readOnly) return;
      const structured = parseSheetClipboardPayload(
        event.clipboardData.getData(SHEET_CLIPBOARD_MIME),
      );
  ...
  const findMatch = useCallback(
    (query: string, options: SheetSearchOptions, direction: 'next' | 'previous') => {
      if (!worksheet || !query) {
  ...
  const replaceMatch = useCallback(
    (query: string, replacement: string, options: SheetSearchOptions) => {
      mutate((current, id) =>
        replaceSheetMatch(current, id, selection.active, query, replacement, options),
      );
    },
    [mutate, selection.active],
  );
  ...
  const goToRange = useCallback(
    (reference: string) => {
      ...
      const named =
        document && worksheet ? resolveNamedRange(document, worksheet.id, reference) : null;
  ...
  const exportWorkbookAs = useCallback(
    async (format: SheetExportFormat, exportOptions: SheetExportOptions = {}) => {
      if (!document) return;
      try {
  ...
  const exportSelection = useCallback(
    ...
    [dateFormat, document, formulaState.values, relativePath, selection, timeFormat, worksheet],
  );
  ...
  const printSelection = useCallback(() => {
    ...
    const baseName = getDocumentBaseName(relativePath, 'Workbook').replace(/\.sheet$/i, '');
    printSheetRange(
      buildSheetRangePrintHtml(document, worksheet, selection, {
        computedValues: formulaState.values,
        displayFormat: { dateFormat, timeFormat },
  ...
  const insertSelectionInNote = useCallback(async () => {
    ...
    try {
      ...
      const paths = new Set(
        flattenVaultFiles(fileTree).map((file) => file.relativePath.toLowerCase()),
      );
  ...
  const loadConnectionSnapshot = useCallback(
    async (connection: SheetDataConnection) => {
  ...
  const addKanbanSnapshot = useCallback(
    async (sourcePath: string) => {
      ...
      try {
        ...
        mutate((current, id) => applySheetDataSnapshot(current, id, selection.active, snapshot));
  ...
  const addCalendarSnapshot = useCallback(
    async (calendarId: string) => {
      if (!worksheet) return;
      try {
        const snapshot = calendarItemSnapshot(await listCalendarItems(calendarId), calendarId);
        mutate((current, id) => applySheetDataSnapshot(current, id, selection.active, snapshot));
  ...
  const refreshDataSnapshot = useCallback(
    async (connection: SheetDataConnection) => {
      try {
    ...
    <div className="flex h-full flex-col overflow-hidden bg-background">
        ...
        secondary={
          <>
            ...
            <div className={documentTopBarGroupClass}>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start">
                  <DropdownMenuItem
                    onSelect={() => mutate((current, id) => trimSheetText(current, id, selection))}
                  >
                    Trim whitespace
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onSelect={() =>
                      mutate((current, id) => splitSheetTextToColumns(current, id, selection))
                    }
                  >
                    Split text by comma
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onSelect={() =>
                      mutate((current, id) => removeDuplicateSheetRows(current, id, selection))
                    }
                  >
            ...
            <div className={documentTopBarGroupClass}>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start">
                  <DropdownMenuItem
                    disabled={!document}
                    onSelect={() => {
                      void exportWorkbookAs('xlsx');
                    }}
                  >
                    <FileSpreadsheet />
                    Export workbook as .xlsx
                  </DropdownMenuItem>
                  <DropdownMenuItem disabled={!document} onSelect={() => setCsvExportOpen(true)}>
                    <FileSpreadsheet />
                    Export as .csv…
                  </DropdownMenuItem>
                  <DropdownMenuItem
              ...
              <DocumentTopBarButton
                onClick={() =>
                  mutate((current, id) => insertTracks(current, id, 'row', bounds.rowCount, 100))
                }
                disabled={session.readOnly || !document}
              >
              ...
              <DocumentTopBarButton
                onClick={() =>
                  mutate((current, id) =>
                    insertTracks(current, id, 'column', bounds.columnCount, 10),
                  )
                }
                disabled={session.readOnly || !document}
              >
            ...
            <div className={documentTopBarGroupClass}>
              <DocumentTopBarButton
                onClick={() =>
                  session.save().catch((error) => toast.error(`Failed to save workbook: ${error}`))
                }
      ...
      {!session.loading && !session.error && document && worksheet && (
        <>
          <SheetFormattingToolbar
            ...
            disabled={session.readOnly}
            onPatch={(patch) =>
              mutate((current, id) => applyStyleToSelection(current, id, selection, patch))
            }
            onClear={() =>
              mutate((current, id) => clearStylesFromSelection(current, id, selection))
            }
            onCopy={() => {
            ...
            onPaste={(mode) => {
              void pasteFromSystemClipboard(mode);
            }}
            onNoteChange={(note) =>
              mutate((current, id) => setCellNote(current, id, selection.active, note))
            }
          />
          <SheetFormulaBar
            selection={selection}
          ...
          <SheetGrid
            ...
            onResizeTrack={(axis, index, size) =>
              mutate((current, id) => resizeTrack(current, id, axis, index, size))
            }
            onAutoSizeColumn={(index) =>
              mutate((current, id) =>
                autoSizeColumn(
                  current,
                  id,
            ...
            scrollPosition={
              initialViewStateRef.current
                ? {
                    top: initialViewStateRef.current.scrollTop,
                    left: initialViewStateRef.current.scrollLeft,
                  }
                : undefined
            }
          ...
          <SheetWorksheetBar
            ...
            onRename={(worksheetId, name) =>
              mutate((current) => renameWorksheet(current, worksheetId, name))
            }
            onDuplicate={(worksheetId) =>
              mutate((current) => duplicateWorksheet(current, worksheetId))
            }
            onDelete={(worksheetId) => mutate((current) => removeWorksheet(current, worksheetId))}
            onReorder={(worksheetId, toIndex) =>
              mutate((current) => reorderWorksheet(current, worksheetId, toIndex))
            }
            onToggleHidden={(worksheetId, hidden) =>
              mutate((current) => setWorksheetHidden(current, worksheetId, hidden))
            }
          />
          ...
          >
            ...
            {summary && summary.filled > 0 && (
              <span>Filled: {summary.filled.toLocaleString()}</span>
            )}
            ...
            {isFrozen && (
              <span>
                Frozen: {frozen.rows}R × {frozen.columns}C
              </span>
            )}
            {formulaState.calculating && <span>Calculating…</span>}
            {validationIssues > 0 && (
              <span className="text-amber-500">Invalid: {validationIssues.toLocaleString()}</span>
            )}
      ...
      <SheetCsvExportDialog
        ...
        onExport={(exportOptions) => {
          void exportWorkbookAs('csv', exportOptions);
        }}
      />
      ...
      <SheetTableDialog
        ...
        onCreate={(name, hasHeaderRow) =>
          mutate((current, id) => createSheetTable(current, id, selection, name, hasHeaderRow))
        }
        onRemove={(tableId) => mutate((current, id) => removeSheetTable(current, id, tableId))}
      />
      {worksheet && (
        <SheetValidationDialog
          ...
          onApply={(validation) =>
            mutate((current, id) => applySheetValidation(current, id, selection, validation))
          }
          onClear={() => mutate((current, id) => clearSheetValidation(current, id, selection))}
        />
      )}
      {document && worksheet && (
        <SheetNamedRangeDialog
          ...
          onCreate={(name, scope) =>
            mutate((current, id) => createSheetNamedRange(current, id, selection, name, scope))
          }
          onRemove={(namedRangeId) =>
            mutate((current) => removeSheetNamedRange(current, namedRangeId))
          }
      ...
      {worksheet && (
        <SheetProtectionDialog
          ...
          onProtect={(name) =>
            mutate((current, id) => protectSheetSelection(current, id, selection, name), {
              allowProtectionChange: true,
            })
          }
          onRemove={(id) =>
            mutate((current, worksheetId) => removeSheetProtection(current, worksheetId, id), {
              allowProtectionChange: true,
            })
          }
        />
      ...
      {worksheet && (
        <SheetConditionalFormatDialog
          ...
          onApply={(draft) =>
            mutate((current, id) => applySheetConditionalFormat(current, id, selection, draft))
          }
          onRemove={(formatId) =>
            mutate((current, id) => removeSheetConditionalFormat(current, id, formatId))
          }
        />
      )}
      {worksheet && (
        <SheetAnalysisDialog
          ...
          onUpsertChart={(chart) => mutate((current, id) => upsertSheetChart(current, id, chart))}
          onRemoveChart={(chartId) =>
            mutate((current, id) => removeSheetChart(current, id, chartId))
          }
          onExportChart={(chart) => {
            void exportChart(chart);
          }}
        />
      )}
      {worksheet && (
        <SheetLinksDialog
          ...
          onSave={(link, attachments) =>
            mutate((current, id) => setCellLinks(current, id, selection.active, link, attachments))
          }
        />
      )}
      {document && (
        <SheetDataConnectionsDialog
          ...
          onAddKanban={(path) => {
            void addKanbanSnapshot(path);
          }}
          onAddCalendar={(calendarId) => {
            void addCalendarSnapshot(calendarId);
          }}
          onRefresh={(connection) => {
            void refreshDataSnapshot(connection);
          }}
          onRemove={(connectionId) =>
            mutate((current) => removeSheetDataConnection(current, connectionId))
          }
        />
      )}
    </div>
