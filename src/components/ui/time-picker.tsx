...
function usesTwelveHourClock(format: TimeFormat): boolean {
  ...
  const options = new Intl.DateTimeFormat(undefined, {
    hour: 'numeric',
  }).resolvedOptions() as Intl.ResolvedDateTimeFormatOptions & { hour12?: boolean };
    ...
    <div className="space-y-1">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>{label ?? 'Choose time'}</DialogTitle>
          </DialogHeader>
          <div className="flex items-center justify-center gap-2 py-4">
            ...
            {twelveHour && (
              <Select value={period} onValueChange={setPeriod}>
                <SelectTrigger aria-label="Period" className="w-24 text-lg">
                  <SelectValue />
                </SelectTrigger>
    ...
    <div className="flex h-8 w-24 overflow-hidden rounded-lg border border-input bg-background dark:bg-input/30">
      ...
      <div className="grid w-7 shrink-0 grid-rows-2 border-l border-input">
        <Button
          ...
          onClick={onIncrease}
          className="h-4 min-h-0 w-7 rounded-none p-0"
        >
        ...
        <Button
          ...
          onClick={onDecrease}
          className="h-4 min-h-0 w-7 rounded-none border-t border-input p-0"
        >
