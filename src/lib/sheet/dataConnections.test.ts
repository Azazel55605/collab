...
describe('sheet data snapshots', () => {
  it('materializes and refreshes a Kanban task snapshot without leaving stale rows', () => {
    ...
    const first = kanbanTaskSnapshot(
      JSON.stringify({
        columns: [
          {
            id: 'todo',
    ...
    const second = kanbanTaskSnapshot(
      JSON.stringify({
        columns: [
          {
            id: 'todo',
    ...
    expect(
      worksheet.cells[sheetCellKey(worksheet.rowOrder[1], worksheet.columnOrder[0])].value,
    ).toBe('Two updated');
    expect(
      worksheet.cells[sheetCellKey(worksheet.rowOrder[2], worksheet.columnOrder[0])],
    ).toBeUndefined();
  });
