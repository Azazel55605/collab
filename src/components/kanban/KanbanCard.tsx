...
import { type DateFormat, formatDate, useUiStore } from '../../store/uiStore';
import {
...
const PRIORITY_BADGE: Record<
  NonNullable<KanbanCard['priority']>,
      ...
      <div className="p-2.5">
        ...
        {(card.priority || card.tags.length > 0) && (
          <div className="flex flex-wrap items-center gap-1 mb-1.5">
            {card.priority &&
              (() => {
                ...
                return (
                  <span
                    className={cn(
                      'flex items-center gap-1 text-[10px] font-semibold px-1.5 py-0.5 rounded border leading-none',
                      p.cls,
                    )}
                  >
                    {p.icon}
            ...
            {card.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="text-[10px] px-1.5 py-0.5 bg-primary/15 text-primary/80 rounded-full leading-none"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="flex items-start gap-1.5">
          ...
          <button
            ...
            onMouseEnter={(e) =>
              !card.isDone && ((e.currentTarget as HTMLElement).style.color = colColor)
            }
            onMouseLeave={(e) =>
              !card.isDone && ((e.currentTarget as HTMLElement).style.color = '')
            }
            ...
            <div className="flex-1 h-1 bg-muted/40 rounded-full overflow-hidden">
              <div
                className={cn(
                  ...
                  checklistPercent < 100 && 'bg-primary/50',
                )}
                style={{
              ...
              {card.dueDate && (
                <span
                  className={cn(
                    ...
                    isOverdue && !card.isDone && 'text-destructive',
                  )}
                >
                  <Calendar size={10} />
...
function SortableKanbanCard({ card, columnId }: Props) {
  const { knownUsers, updateBoard, board, relativePath, caps, remoteCardEditors } =
    useKanbanContext();
  ...
  const moveCardToColumn = useCallback(
    (destColId: string, options?: { forceDone?: boolean; autoApplyTags?: boolean }) => {
        ...
        if (missingTags.length > 0 && !shouldAutoApplyTags) {
          window.setTimeout(() => {
            window.dispatchEvent(
              new CustomEvent('kanban:prompt-move-tags', {
                detail: {
                  cardId: moved.id,
  ...
  const onToggleDone = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (willBeDone) {
        ...
        if (dests.length === 1) {
          moveCardToColumn(dests[0].id, { forceDone: true });
          return;
        }
        if (dests.length > 1) {
          setDestPicker(dests);
  ...
  return (
    <>
                ...
                {board.columns
                  ...
                  .map((col) => (
                    <ContextMenuItem
                      key={col.id}
                      className="text-xs"
                      onSelect={() => moveCardToColumn(col.id)}
                    >
                      <div
      ...
      {dialogOpen && <CardDialog card={card} columnId={columnId} onClose={clearEditing} />}
