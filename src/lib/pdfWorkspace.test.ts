...
describe('pdfWorkspace', () => {
  ...
  it('adds quote text nodes to canvas data', () => {
    ...
    const result = appendPdfQuoteTextNode(
  ...
  it('adds snapshot file nodes to canvas data', () => {
    ...
    const result = appendPdfSnapshotFileNode(
      { nodes: [], edges: [], viewport: { x: 0, y: 0, zoom: 1 } },
      'Pictures/page.png',
    );
    expect(result.nodes).toHaveLength(1);
    expect(result.nodes[0]).toMatchObject({
      ...
      type: 'file',
      relativePath: 'Pictures/page.png',
    });
  });

  it('builds snapshot markdown with source line', () => {
    expect(buildPdfSnapshotMarkdown('Docs/spec.pdf', 2, 'Pictures/page.png')).toContain(
      'Source: Docs/spec.pdf (page 2)',
    );
  });
});
