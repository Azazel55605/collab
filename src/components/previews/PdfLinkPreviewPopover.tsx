...
export function PdfLinkPreviewPopover({
  anchorRect,
  relativePath,
  enabled,
}: PdfLinkPreviewPopoverProps) {
  ...
  return createPortal(
    <div className="pointer-events-none fixed z-[120] w-[280px]" style={{ top, left }}>
        ...
        {previewSrc ? (
          <div className="bg-white/90 p-2">
            <img
              src={previewSrc}
              alt=""
              className="max-h-44 w-full rounded-lg border border-border/50 object-contain"
              draggable={false}
            />
          </div>
        ) : (
