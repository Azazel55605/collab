...
import { createSelection, selectAll, selectColumns, selectRows, extendSelection } from './selection';
...
describe('describeSheetCell', () => {
  it('reads the address and the displayed content', () => {
    expect(describeSheetCell({ worksheet, position: { row: 0, column: 0 } }))
      .toBe('A1, Rent');
    expect(describeSheetCell({ worksheet, position: { row: 0, column: 1 } }))
      .toBe('B1, 1240');
  });

  it('says a blank cell is empty rather than saying nothing', () => {
    expect(describeSheetCell({ worksheet, position: { row: 2, column: 2 } }))
      .toBe('C3, empty');
  });
  ...
  it('announces a formula whose value has not been computed yet', () => {
    expect(describeSheetCell({ worksheet, position: { row: 1, column: 0 } }))
      .toContain('formula =A1');
  });

  it('mentions links and attachments', () => {
    expect(describeSheetCell({ worksheet, position: { row: 1, column: 1 } }))
      .toContain('has link');
    expect(describeSheetCell({ worksheet, position: { row: 2, column: 0 } }))
...
describe('describeSheetSelection', () => {
  it('reads the cell itself for a single-cell selection', () => {
    ...
    expect(describeSheetSelection({ worksheet, selection, position: selection.active }))
      .toBe('A1, Rent');
  });

  it('describes the shape of a range instead of every cell in it', () => {
    const selection = extendSelection(createSelection({ row: 0, column: 0 }), { row: 2, column: 2 });
  ...
  it('reports how many disjoint ranges are selected', () => {
    ...
    expect(describeSheetSelection({ worksheet, selection, position: selection.active }))
      .toContain('2 ranges selected, 2 cells');
  });
