...
function ItemTypeIcon({ item, className }: { item: CalendarItem; className?: string }) {
  if (item.kind === 'birthday')
    return <Gift role="img" aria-label="Birthday" className={className} />;
  if (item.kind === 'task' && item.sourceBinding?.kind === 'kanban')
    return <SquareKanban role="img" aria-label="Kanban task" className={className} />;
  if (item.kind === 'task')
    return <ClipboardCheck role="img" aria-label="Task" className={className} />;
...
function DateContextMenu({
  ...
  return (
    <ContextMenu>
      ...
      <ContextMenuContent>
        <ContextMenuItem onSelect={() => onAdd({ date, kind: 'event' })}>
          <CalendarDays /> Add event
        </ContextMenuItem>
        <ContextMenuItem onSelect={() => onAdd({ date, kind: 'task' })}>
          <ClipboardCheck /> Add task
        </ContextMenuItem>
        <ContextMenuItem onSelect={() => onAdd({ date, kind: 'birthday' })}>
          <Gift /> Add birthday
        </ContextMenuItem>
      </ContextMenuContent>
    </ContextMenu>
...
export default function CalendarPage() {
  ...
  const [subscriptionDeleteRequest, setSubscriptionDeleteRequest] =
  ...
  const [periodTransition, setPeriodTransition] = useState<{ direction: -1 | 1; sequence: number }>(
    { direction: 1, sequence: 0 },
  );
  ...
  const range = useMemo(() => {
    if (viewMode === 'year')
      return { from: `${anchor.getFullYear()}-01-01`, to: `${anchor.getFullYear() + 1}-01-01` };
    if (viewMode === 'week')
      return {
        from: calendarDateKey(fullWeekDays[0]),
        to: calendarDateKey(addDays(fullWeekDays[6], 1)),
      };
    if (viewMode === 'day' || viewMode === 'agenda')
      return { from: selectedDate, to: nextDateKey(selectedDate) };
  ...
  const hostedOrigins = useMemo(
    () =>
      Object.values(serverConnections).flatMap((connection) => {
  ...
  useEffect(() => {
    void store.initialize(profileId);
  }, [profileId, store.initialize]);
  useEffect(() => {
  ...
  useEffect(
    () => () => {
      if (horizontalGestureReset.current != null)
        window.clearTimeout(horizontalGestureReset.current);
    },
    [],
  );
  ...
  const calendarById = useMemo(
    () => new Map(store.calendars.map((calendar) => [calendar.id, calendar])),
    [store.calendars],
  );
  ...
  const calendarLocations = useMemo(
    () => [
      ...
      ...Object.values(serverConnections)
        .filter(
          (connection) =>
            connection.status.connected && connection.status.serverUrl && connection.status.user,
        )
        .map((connection) => ({
  ...
  const editItem = (item: CalendarItem) =>
    openEditor({ date: itemDate(item), kind: item.kind, item });
  ...
  const kanbanEditorItem =
    editorRequest?.item?.kind === 'task' && editorRequest.item.sourceBinding?.kind === 'kanban'
      ? editorRequest.item
      : null;
  ...
  return (
    ...
    >
      <header className="flex min-h-12 shrink-0 flex-wrap items-center gap-2 border-b border-border/60 px-3 py-2">
        ...
        <div className="min-w-28 flex-1 truncate text-center text-sm font-medium">
          {viewTitle(viewMode, anchor, selectedDay, weekDays)}
        </div>
        <CalendarSearch
          open={searchOpen}
        ...
        >
          {VIEW_MODES.map((mode) => (
            <button
              ...
              className={cn(
                'h-6 rounded px-2 text-[11px] text-muted-foreground',
        ...
        {hostedOrigins.length > 0 && (
          <CalendarInvitations
            origins={hostedOrigins}
            onChanged={() => void store.syncHosted(hostedOrigins)}
          />
        )}
        {hostedOrigins.length > 0 && (
          <Button
            variant="ghost"
            size="icon-sm"
            aria-label="Manage CalDAV access"
            title="Manage CalDAV access"
        ...
        {hostedOrigins.length > 0 && (
          <Button
            variant="ghost"
            size="icon-sm"
            aria-label="Sync hosted calendars"
            title="Sync hosted calendars"
            disabled={store.syncing}
            onClick={() => void store.syncHosted(hostedOrigins)}
          >
            <RefreshCw className={cn(store.syncing && 'animate-spin')} />
          </Button>
        )}
        <Button
          size="sm"
          onClick={() => openEditor({ date: selectedDate, kind: 'event' })}
          disabled={store.calendars.length === 0}
        >
          <Plus /> New
        </Button>
      </header>

      {(store.error || interopError) && (
        <button
          type="button"
          className="shrink-0 border-b border-destructive/30 bg-destructive/10 px-3 py-1.5 text-left text-xs text-destructive"
          onClick={() => {
            store.clearError();
            setInteropError(null);
          }}
      ...
      <div className="flex min-h-0 flex-1">
        {showCalendarRail && (
          <CalendarRail
            ...
            mirrorAttention={store.mirrorStatuses.some(
              (status) =>
            ...
            onRefreshSubscription={(subscriptionId) =>
              void store.refreshSubscription(subscriptionId).catch(() => {})
            }
            onDeleteSubscription={(subscriptionId) =>
              setSubscriptionDeleteRequest(
                store.subscriptions.find((entry) => entry.id === subscriptionId) ?? null,
              )
            }
        ...
        <main
          className={cn(
            ...
            showMonthAgenda && 'grid-cols-[minmax(0,1fr)_240px]',
          )}
        >
          <div
            data-calendar-scroll
            className="h-full min-h-0 min-w-0 overflow-auto overscroll-contain"
            onWheel={handleWheel}
          >
            <div
              key={periodTransition.sequence}
              className={cn(
                'h-full min-h-0',
                periodTransition.sequence > 0 && 'calendar-period-transition',
                periodTransition.direction > 0
                  ? 'calendar-period-transition-next'
                  : 'calendar-period-transition-previous',
              )}
            >
              {viewMode === 'month' && (
                <MonthView
              ...
              {viewMode === 'week' && (
                <WeekView
                  ...
                  onAllDayReschedule={rescheduleAllDayItem}
                  onResize={resizeItem}
                />
              )}
              {viewMode === 'day' && (
      ...
      <ItemEditorDialog
        request={kanbanEditorItem ? null : editorRequest}
        onOpenChange={(open) => !open && setEditorRequest(null)}
        calendars={editableCalendars}
        saving={store.saving}
        onSave={store.saveItem}
      />
      <KanbanTaskEditorDialog
        item={kanbanEditorItem}
        saving={store.saving}
        onOpenChange={(open) => !open && setEditorRequest(null)}
        onSave={(item) => store.saveItem(item, 'series')}
      />
      <RecurringDeleteDialog
        ...
        onDelete={async (scope) => {
          ...
          await store.deleteItem(deleteRequest, scope);
          setDeleteRequest(null);
        }}
      />
      <RecurringRescheduleDialog
        item={rescheduleRequest?.item ?? null}
        saving={store.saving}
        onOpenChange={(open) => !open && setRescheduleRequest(null)}
        onSave={async (scope) => {
          if (!rescheduleRequest) return;
          await store.saveItem(rescheduleRequest.item, scope);
      ...
      />
      <CalendarPublishDialog
        calendar={publishCalendar}
        onOpenChange={(open) => !open && setPublishCalendar(null)}
      />
      ...
      />
      <Dialog
        open={subscriptionDeleteRequest != null}
        onOpenChange={(open) => !open && setSubscriptionDeleteRequest(null)}
      >
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>Remove calendar subscription?</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground">
            The cached read-only calendar and its entries will be removed from this profile. The
            external feed is not changed.
          </p>
          <DialogFooter>
            <Button
              type="button"
              variant="outline"
              onClick={() => setSubscriptionDeleteRequest(null)}
            >
              Cancel
            </Button>
            <Button
              ...
              onClick={() => {
                if (!subscriptionDeleteRequest) return;
                void store
                  .deleteSubscription(subscriptionDeleteRequest.id)
                  .then(() => setSubscriptionDeleteRequest(null))
                  .catch(() => {});
              }}
            >
              {store.saving ? 'Removing...' : 'Remove'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      <CalendarMirrorDialog
        ...
        onResolve={(conflictId, memberId) =>
          store.resolveMirrorConflict(conflictId, memberId, hostedOrigins)
        }
      />
      ...
      <CalendarExportDialog
        ...
        onExport={async (calendar, items, suffix) => {
          const safeName =
            calendar.name.replace(/[<>:"/\\|?*\u0000-\u001f]/g, '-').trim() || 'calendar';
...
function CalendarExportDialog({
  ...
  onExport: (calendar: CalendarDefinition, items: CalendarItem[], suffix: string) => Promise<void>;
}) {
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-3">
          <label className="block space-y-1">
            <span className="text-xs font-medium">Export scope</span>
            <Select value={scope} onValueChange={(value) => setScope(value as typeof scope)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="calendar">Entire calendar</SelectItem>
                <SelectItem value="range">Visible range</SelectItem>
                <SelectItem value="day">Selected day</SelectItem>
                <SelectItem value="items">Selected items</SelectItem>
              </SelectContent>
            </Select>
          </label>
          {scope === 'items' && (
            <div className="max-h-56 space-y-1 overflow-y-auto rounded-md border p-1">
              {request.items.map((item) => (
                ...
                >
                  ...
                  />
                  <ItemTypeIcon item={item} className="size-3 shrink-0" />
                  <span className="min-w-0 flex-1 truncate">{item.title}</span>
                </label>
              ))}
              {request.items.length === 0 && (
                <p className="px-2 py-3 text-xs text-muted-foreground">No items are available.</p>
              )}
            </div>
          )}
          <p className="text-xs text-muted-foreground">
            {output.length} item{output.length === 1 ? '' : 's'} will be written to the iCalendar
            file.
          </p>
        </div>
        <DialogFooter>
          ...
          <Button
            type="button"
            disabled={scope === 'items' && output.length === 0}
            onClick={() => void onExport(request.calendar, output, suffix)}
          >
            <FileDown />
            Export
          </Button>
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="space-y-2">
          <p className="text-sm text-muted-foreground">
            ...
            <span className="font-medium text-foreground">{item?.title}</span> to delete.
          </p>
          <Select
            value={scope}
            onValueChange={(value) => setScope(value as CalendarRecurrenceEditScope)}
          >
            <SelectTrigger aria-label="Recurring delete scope">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="occurrence">This occurrence</SelectItem>
              <SelectItem value="following">This and following occurrences</SelectItem>
...
function RecurringRescheduleDialog({
  item,
  saving,
  onOpenChange,
  onSave,
  ...
  return (
    <Dialog open={item != null} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          <DialogTitle>Reschedule recurring item</DialogTitle>
        </DialogHeader>
        <div className="space-y-2">
          <p className="text-sm text-muted-foreground">
            Choose which occurrences of{' '}
            <span className="font-medium text-foreground">{item?.title}</span> to reschedule.
          </p>
          <Select
            value={scope}
            onValueChange={(value) => setScope(value as CalendarRecurrenceEditScope)}
          >
            <SelectTrigger aria-label="Recurring reschedule scope">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="occurrence">This occurrence</SelectItem>
        ...
        <DialogFooter>
          ...
          <Button type="button" disabled={saving} onClick={() => void onSave(scope)}>
            {saving ? 'Saving...' : 'Save'}
          </Button>
        </DialogFooter>
      </DialogContent>
...
export function CalendarRail({
  ...
  return (
    <aside className="flex w-44 shrink-0 flex-col border-r border-border/60 bg-sidebar/40 p-3">
      <div className="mb-2 flex items-center justify-between">
        <span className="text-[10px] font-semibold uppercase text-muted-foreground">Calendars</span>
        <span className="flex items-center">
          <Button
            variant="ghost"
            size="icon-xs"
            aria-label="Manage calendar mirrors"
            title="Manage calendar mirrors"
            className="relative"
            onClick={onMirrors}
          >
            <Link2 />
          ...
          <Button variant="ghost" size="icon-xs" aria-label="Add calendar" onClick={onAdd}>
            <CirclePlus />
          </Button>
        </span>
      </div>
      <div className="min-h-0 flex-1 space-y-1 overflow-y-auto">
        {activeCalendars.map((calendar) => {
          ...
          return (
            <div
              key={calendar.id}
              className="group flex items-center gap-1 rounded-md px-1.5 py-1.5 text-xs hover:bg-accent/60"
            >
              <Checkbox
                id={checkboxId}
                checked={visibleIds.includes(calendar.id)}
                onCheckedChange={(checked) => onVisible(calendar.id, checked === true)}
              />
              <label
                htmlFor={checkboxId}
                className="flex min-w-0 flex-1 cursor-pointer items-center gap-2"
                title={subscription?.lastError || undefined}
              >
                <span
        ...
        {archivedCalendars.length > 0 && (
          <>
            <div className="mb-1 mt-3 px-1 text-[9px] font-semibold uppercase text-muted-foreground">
              Archived
            </div>
            {archivedCalendars.map((calendar) => (
              <div
                key={calendar.id}
                className="group flex items-center gap-1 rounded-md px-1.5 py-1.5 text-xs text-muted-foreground hover:bg-accent/60"
              >
                <span
                ...
                <Button
                  ...
                  variant="ghost"
                  size="icon-xs"
                  disabled={saving || calendar.readOnly}
                  aria-label={`Restore ${calendar.name}`}
                  title="Restore calendar"
                  onClick={() => onArchive(calendar, false)}
                >
                  <ArchiveRestore />
                </Button>
                <CalendarActions
                  calendar={calendar}
                  subscription={subscriptions.find((entry) => entry.calendarId === calendar.id)}
                  saving={saving}
                  onEdit={onEdit}
                  onImport={onImport}
                  onExport={onExport}
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-40">
        <DropdownMenuItem disabled={saving || calendar.readOnly} onSelect={() => onEdit(calendar)}>
          <Pencil />
          Edit
        </DropdownMenuItem>
        <DropdownMenuItem
          disabled={saving || calendar.readOnly}
          onSelect={() => onImport(calendar)}
        >
          <FileUp />
          Import .ics
        </DropdownMenuItem>
        <DropdownMenuItem disabled={saving} onSelect={() => onExport(calendar)}>
          <FileDown />
          Export .ics
        </DropdownMenuItem>
        {calendar.location.kind === 'hosted' && (
          <DropdownMenuItem disabled={saving} onSelect={() => onPublish(calendar)}>
            <Rss />
            Publish feed
          </DropdownMenuItem>
        )}
        {subscription && (
          <DropdownMenuItem
            disabled={saving}
            onSelect={() => onRefreshSubscription(subscription.id)}
          >
            <RefreshCw />
            Refresh feed
          </DropdownMenuItem>
        ...
        {subscription && (
          <DropdownMenuItem
            variant="destructive"
            disabled={saving}
            onSelect={() => onDeleteSubscription(subscription.id)}
          >
            <Trash2 />
            Remove subscription
          </DropdownMenuItem>
        ...
        {!subscription && (
          <DropdownMenuItem
            disabled={saving || calendar.readOnly}
            onSelect={() => onArchive(calendar, !calendar.archived)}
          >
            ...
            results.map((item) => {
              ...
              return (
                <button
                  ...
                  className="flex w-full items-center gap-2 rounded-md px-2 py-2 text-left hover:bg-accent focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary"
                >
                  <span className="flex size-6 shrink-0 items-center justify-center rounded-md bg-muted">
                    <ItemTypeIcon item={item} className="size-3.5 text-muted-foreground" />
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-xs font-medium">{item.title}</span>
                    <span className="block truncate text-[10px] text-muted-foreground">
                      {itemDate(item)}
                      {calendar ? ` · ${calendar.name} · ${calendarOrigin(calendar)}` : ''}
                    </span>
                  </span>
                  {calendar && (
                    <span
...
function MonthView({
  ...
  const labels = Array.from(
    { length: 7 },
    (_, index) => new Date(2026, 6, 5 + ((weekStart + index) % 7)),
  )
    .filter((day) => !hideWeekends || (day.getDay() !== 0 && day.getDay() !== 6))
    .map((day) => new Intl.DateTimeFormat(undefined, { weekday: 'short' }).format(day));
  return (
    <section className="grid h-full min-h-[570px] grid-rows-[28px_repeat(6,minmax(90px,1fr))] overflow-hidden">
      <div
      ...
      >
        {labels.map((label) => (
          <div
            key={label}
            className="px-2 py-1 text-right text-[10px] font-medium uppercase text-muted-foreground"
          >
            {label}
          </div>
        ))}
        ...
        >
          {days
            ...
            .map((day) => {
              ...
              return (
                <DateContextMenu key={key} date={key} onAdd={onAdd}>
                  <div
                    role="button"
                    tabIndex={0}
                    ...
                    className={cn(
                      'min-h-0 overflow-hidden border-b border-r border-border/50 p-1 text-left hover:bg-accent/30 focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary',
                      day.getMonth() !== anchor.getMonth() &&
                        'bg-muted/10 text-muted-foreground/60',
                      key === selectedDate && 'bg-primary/5 ring-1 ring-inset ring-primary/40',
                    )}
                  >
                    <span
                      className={cn(
                        'ml-auto flex size-5 items-center justify-center rounded-full text-[11px]',
                        key === calendarDateKey(new Date()) &&
...
function WeekView({
  ...
  const allDayItems = days.map((day) => {
    const key = calendarDateKey(day);
    return items.filter(
      (item) => itemOccursOn(item, day) && !layoutCalendarTimedItems([item], key).length,
    );
  });
  ...
  return (
    <section ref={rootRef} className={cn(days.length === 7 ? 'min-w-[760px]' : 'min-w-[560px]')}>
      <div
      ...
      >
        ...
        {days.map((day) => {
          ...
          return (
            <button
              ...
              onClick={() => onSelect(key)}
              className={cn(
                'border-r border-border/60 py-1.5 text-center hover:bg-accent/40',
        ...
        <div
          className="flex items-start justify-end border-r border-t border-border/60 px-2 pt-2 text-[9px] uppercase text-muted-foreground"
...
function DayView({
  ...
  return (
    <section ref={rootRef} className="mx-auto min-w-[520px] max-w-5xl">
...
function TimeGutter({ hourLabel }: { hourLabel: (hour: number) => string }) {
  return (
    <div className="relative border-r border-border/60" style={{ height: WEEK_HOUR_HEIGHT * 24 }}>
...
function AllDayDropLane({
  ...
  return (
    <DateContextMenu date={dateKey} onAdd={onAdd}>
      <div
        className={cn(
        ...
        style={{ height }}
        onDragOver={(event) => {
          if (!Array.from(event.dataTransfer.types).includes(CALENDAR_ALL_DAY_ITEM_DRAG_TYPE))
            return;
          event.preventDefault();
        ...
        onDrop={(event) => {
          ...
          setDropActive(false);
          try {
            const { itemId, sourceDateKey } = JSON.parse(payload) as {
              itemId: string;
      ...
      >
        <div className="space-y-0.5">
          ...
          {dayItems.length > 3 && (
            <div className="px-1 text-[9px] text-muted-foreground">+{dayItems.length - 3} more</div>
          )}
        </div>
      </div>
    </DateContextMenu>
...
function TimedDayColumn({
  ...
  const [resizePreview, setResizePreview] = useState<{ itemId: string; endMinute: number } | null>(
    null,
  );
  const timedItems = layoutCalendarTimedItems(items, dateKey);
  const minuteAtPointer = (clientY: number, top: number) =>
    snapCalendarMinute(((clientY - top) / WEEK_HOUR_HEIGHT) * 60);
  useEffect(() => {
    ...
    <DateContextMenu date={dateKey} onAdd={onAdd}>
      <div
        ...
        onDragOver={(event) => {
          ...
          event.dataTransfer.dropEffect = 'move';
          setDropMinute(
            minuteAtPointer(event.clientY, event.currentTarget.getBoundingClientRect().top),
          );
        }}
        onDragLeave={(event) => {
          if (!event.currentTarget.contains(event.relatedTarget as Node | null))
      ...
      >
        ...
        {dropMinute != null && (
          <div
            className="pointer-events-none absolute left-0 right-0 z-[5] border-t-2 border-primary"
            style={{ top: (dropMinute / 60) * WEEK_HOUR_HEIGHT }}
          />
        )}
        {dateKey === todayKey && (
          <div
            className="pointer-events-none absolute left-0 right-0 z-10 border-t border-primary"
            style={{ top: (nowMinute / CALENDAR_MINUTES_PER_DAY) * WEEK_HOUR_HEIGHT * 24 }}
          >
        ...
        {timedItems.map((entry) => {
          ...
          const previewEndMinute =
            resizePreview?.itemId === entry.item.id ? resizePreview.endMinute : entry.endMinute;
          const canResize =
            ...
            >
              <div
                ...
                style={{
                  ...
                  height: Math.max(
                    22,
                    ((previewEndMinute - entry.startMinute) / 60) * WEEK_HOUR_HEIGHT,
                  ),
              ...
              >
                ...
                >
                  <span className="flex min-w-0 items-center gap-1 text-[10px] font-medium">
                    <ItemTypeIcon item={entry.item} className="size-2.5 shrink-0" />
                ...
                {canResize && (
                  ...
                  >
                    <span className="mx-auto block h-0.5 w-7 rounded-full bg-muted-foreground/60" />
                  </button>
                )}
              </div>
            </ItemContextMenu>
...
function AgendaView({
  ...
  const key = calendarDateKey(day);
  return (
    <DateContextMenu date={key} onAdd={onAdd}>
      <section className="mx-auto min-h-full w-full max-w-3xl px-5 py-4">
        <div className="mb-5 border-b border-border/60 pb-3">
          <div className="text-xs uppercase text-muted-foreground">
            {new Intl.DateTimeFormat(undefined, { weekday: 'long' }).format(day)}
          </div>
          <div className="text-xl font-semibold">
            {new Intl.DateTimeFormat(undefined, { month: 'long', day: 'numeric' }).format(day)}
          </div>
        </div>
        <div className="space-y-2">
          {items.map((item) => (
...
function TasksView({
  ...
  onEdit,
  onDelete,
}: ViewProps & { anchor: Date }) {
  const tasks = items
    .filter((item): item is Extract<CalendarItem, { kind: 'task' }> => item.kind === 'task')
    .sort(
      (left, right) =>
        itemDate(left).localeCompare(itemDate(right)) || left.title.localeCompare(right.title),
    );
  ...
  const renderGroup = (title: string, group: typeof tasks) => (
    <section aria-labelledby={`calendar-tasks-${title.toLowerCase().replace(/ /g, '-')}`}>
      <h2
        id={`calendar-tasks-${title.toLowerCase().replace(/ /g, '-')}`}
        className="mb-2 text-[10px] font-semibold uppercase text-muted-foreground"
      >
        {title} <span className="ml-1 font-normal">{group.length}</span>
      </h2>
      <div className="divide-y divide-border/50 border-y border-border/50">
        {group.map((task) => {
          const calendar = calendarById.get(task.calendarId);
          return (
            <ItemContextMenu key={task.id} item={task} onEdit={onEdit} onDelete={onDelete}>
              <button
                type="button"
                onClick={() => onEdit(task)}
                className="flex w-full items-center gap-3 px-2 py-2.5 text-left hover:bg-accent/40 focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary"
              >
                <CheckCircle2
                  className={cn(
                    'size-4 shrink-0',
                ...
                {calendar && (
                  <span className="flex max-w-40 items-center gap-1.5 truncate text-[10px] text-muted-foreground">
                    <span
                      className="size-2 shrink-0 rounded-full"
                      style={{ backgroundColor: calendar.color }}
                    />
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-6 px-5 py-4">
      <div className="flex items-center justify-between">
        ...
        <Button
          type="button"
          size="sm"
          onClick={() => onAdd({ date: calendarDateKey(anchor), kind: 'task' })}
        >
          <Plus />
          New task
        </Button>
...
function YearView({
  ...
  return (
    <section className="grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] gap-px bg-border/50 p-px">
      {Array.from({ length: 12 }, (_, month) => {
        const anchor = new Date(year, month, 1);
        const days = calendarMonthGrid(anchor, weekStart).filter(
          (day) => !hideWeekends || (day.getDay() !== 0 && day.getDay() !== 6),
        );
        return (
          <div key={month} className="bg-background p-3">
            <button
              type="button"
              className="mb-2 text-xs font-semibold hover:text-primary"
              onClick={() => onOpenMonth(anchor)}
            >
              {new Intl.DateTimeFormat(undefined, { month: 'long' }).format(anchor)}
            </button>
            <div
            ...
            >
              {days.map((day) => {
                ...
                const colors = Array.from(
                  new Set(
                    items
                      .filter((item) => itemOccursOn(item, day))
                      .map((item) => calendarById.get(item.calendarId)?.color)
                      .filter(Boolean),
                  ),
                ).slice(0, 3);
                return (
                  <DateContextMenu key={key} date={key} onAdd={onAdd}>
                    <button
                      type="button"
                      onDoubleClick={() => onOpenMonth(day)}
                      className={cn(
                        'relative h-7 text-[10px] hover:bg-accent',
                        day.getMonth() !== month && 'text-muted-foreground/35',
                        key === calendarDateKey(new Date()) &&
...
function ItemChip({
  ...
  return (
    <ItemContextMenu item={item} onEdit={onEdit} onDelete={onDelete}>
      <button
        type="button"
        draggable={draggable}
        onDragStart={
          draggable
            ? (event) => {
                ...
                event.dataTransfer.setData(
                  CALENDAR_ALL_DAY_ITEM_DRAG_TYPE,
                  JSON.stringify({
                    itemId: item.id,
        ...
        className={cn(
          'flex w-full min-w-0 items-center gap-1 truncate rounded-sm bg-muted/75 px-1 py-0.5 text-left text-[10px] hover:bg-accent',
          draggable && 'cursor-grab active:cursor-grabbing',
        )}
      >
        <span className="size-1.5 shrink-0 rounded-full" style={{ backgroundColor: color }} />
        <ItemTypeIcon item={item} className="size-2.5 shrink-0" />
        <span className="truncate">
          {showTime && `${itemTime(item)} `}
          {item.title}
        </span>
      </button>
    </ItemContextMenu>
  ...
  return (
    <ItemContextMenu item={item} onEdit={onEdit} onDelete={onDelete}>
      <button
        type="button"
        onClick={() => onEdit(item)}
        className="flex w-full items-center gap-3 border-l-2 px-3 py-2 text-left hover:bg-accent/40"
        style={{ borderColor: color }}
      >
        <span className="w-16 shrink-0 text-xs text-muted-foreground">{itemTime(item)}</span>
        <ItemTypeIcon item={item} className="size-3.5 shrink-0 text-muted-foreground" />
...
function AgendaPanel({
  ...
  return (
    <aside className="min-h-0 overflow-y-auto border-l border-border/60 p-3">
      ...
      <div className="space-y-1">
        ...
        {items.length === 0 && (
          <div className="py-6 text-center text-xs text-muted-foreground">No entries</div>
        )}
      </div>
    </aside>
  );
...
function ReminderEditor({
  ...
  const selectedMinutes =
    draft === 'custom' ? Math.max(1, Number(customAmount) || 1) * multiplier : Number(draft);
  return (
    <div className="space-y-1.5">
      ...
      {values.map((minutes) => (
        ...
        >
          ...
          <Button
            ...
            onClick={() => onChange(values.filter((value) => value !== minutes))}
          >
            <Trash2 />
          </Button>
        </div>
      ...
      <div className="flex gap-2">
        ...
        <Button
          ...
          variant="outline"
          size="sm"
          disabled={values.includes(selectedMinutes)}
          onClick={() => onChange([...values, selectedMinutes].sort((left, right) => left - right))}
        >
          Add
        </Button>
      </div>
...
export function ItemEditorDialog({
  ...
  calendars: CalendarDefinition[];
  saving: boolean;
  onSave: (item: CalendarItem, scope?: CalendarRecurrenceEditScope) => Promise<CalendarItem>;
}) {
  const timeFormat = useUiStore((state) => state.timeFormat);
  ...
  useEffect(() => {
    ...
    setRelativeReminders(
      (item?.reminders ?? []).flatMap((reminder) =>
        reminder.kind === 'relative' ? [reminder.minutesBefore] : [],
      ),
    );
    ...
    if (item?.kind === 'event') {
      ...
      setStartDate(
        item.start.kind === 'date'
          ? item.start.date
          : calendarDateKey(new Date(item.start.dateTime)),
      );
      setEndDate(
        eventAllDay
          ? calendarDateKey(
              addDays(dateFromKey(item.end.kind === 'date' ? item.end.date : request.date), -1),
            )
          : calendarDateKey(
              new Date(item.end.kind === 'dateTime' ? item.end.dateTime : request.date),
            ),
      );
      if (item.start.kind === 'dateTime')
        setStartTime(new Date(item.start.dateTime).toTimeString().slice(0, 5));
      if (item.end.kind === 'dateTime')
        setEndTime(new Date(item.end.dateTime).toTimeString().slice(0, 5));
      ...
      setStartDate(
        start?.kind === 'date'
          ? start.date
          : start?.kind === 'dateTime'
            ? calendarDateKey(new Date(start.dateTime))
            : request.date,
      );
      setEndDate(
        due?.kind === 'date'
          ? due.date
          : due?.kind === 'dateTime'
            ? calendarDateKey(new Date(due.dateTime))
            : request.date,
      );
      if (start?.kind === 'dateTime')
        setStartTime(new Date(start.dateTime).toTimeString().slice(0, 5));
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const preservedAbsoluteReminders =
      request.item?.reminders.filter((reminder) => reminder.kind === 'absolute') ?? [];
    ...
    const base = {
      id,
      uid: request.item?.uid ?? `${id}@collab.local`,
      calendarId,
      title: title.trim(),
      ...
      attachments,
      reminders: [
        ...relativeReminders.map((minutesBefore) => ({ kind: 'relative' as const, minutesBefore })),
        ...preservedAbsoluteReminders,
      ],
      recurrence,
      ...
      createdAt: request.item?.createdAt ?? now,
      updatedAt: now,
    };
    ...
    const endValue = allDay
      ? { kind: 'date' as const, date: request.kind === 'event' ? nextDateKey(endDate) : endDate }
      : {
          kind: 'dateTime' as const,
    ...
    const item =
      request.kind === 'event'
        ? normalizeCalendarItem({
            ...
            availability: request.item?.kind === 'event' ? request.item.availability : 'busy',
          })
        : request.kind === 'task'
          ? normalizeCalendarItem({
              ...base,
              ...
              due: endValue,
              status: taskStatus,
              priority: taskPriority === 'none' ? undefined : taskPriority,
              completedAt:
                taskStatus === 'completed'
                  ? ((request.item?.kind === 'task' ? request.item.completedAt : undefined) ?? now)
                  : undefined,
            })
          : normalizeCalendarItem({
              ...base,
              kind: 'birthday',
              date: startDate,
              birthYear: request.item?.kind === 'birthday' ? request.item.birthYear : undefined,
            });
  ...
  const updateCalendarId = (nextCalendarId: string) => {
    ...
    const currentServer =
      selectedCalendar?.location.kind === 'hosted' ? selectedCalendar.location.serverUrl : null;
    const nextServer =
      nextCalendar?.location.kind === 'hosted' ? nextCalendar.location.serverUrl : null;
  ...
  return (
    <Dialog open={request != null} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{request?.item ? `Edit ${kindLabel}` : `New ${kindLabel}`}</DialogTitle>
        </DialogHeader>
        <form
          id="calendar-item-form"
          className="space-y-4"
        ...
        >
          ...
          {request?.kind === 'task' && (
            <div className="grid grid-cols-2 gap-2">
              <label className="block space-y-1">
                ...
                <Select
                  value={taskStatus}
                  onValueChange={(value) => setTaskStatus(value as CalendarTaskStatus)}
                >
              ...
              <label className="block space-y-1">
                ...
                <Select
                  value={taskPriority}
                  onValueChange={(value) => setTaskPriority(value as CalendarTaskPriority | 'none')}
                >
          ...
          {isScheduled ? (
            <>
              <div className="flex items-center gap-2">
                <Checkbox
                  id="calendar-item-all-day"
                  checked={allDay}
                  onCheckedChange={(checked) => setAllDay(checked === true)}
                />
                <label
                  htmlFor="calendar-item-all-day"
              ...
              <div className="grid grid-cols-2 gap-2">
                <DatePicker
                  label={request?.kind === 'task' ? 'Starts' : 'Start date'}
                  value={startDate}
                  onChange={setStartDate}
                />
                <DatePicker
                  label={request?.kind === 'task' ? 'Deadline' : 'End date'}
                  value={endDate}
                  min={startDate}
                  onChange={setEndDate}
                />
              ...
              {!allDay && (
                <div className="grid grid-cols-2 gap-2">
                  ...
                  />
                  <TimePicker
                    label={request?.kind === 'task' ? 'Deadline time' : 'End time'}
                    value={endTime}
                    onChange={setEndTime}
                    format={timeFormat}
                  />
              ...
              <label className="block space-y-1">
                ...
                <Select
                  value={repeat}
                  onValueChange={(value) => setRepeat(value as RecurrencePreset)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              {repeat === 'custom' && (
                <label className="block space-y-1">
                  <span className="text-xs font-medium">Recurrence rule</span>
                  <Input
              ...
              {request?.item?.recurrenceId && (
                <label className="block space-y-1">
                  <span className="text-xs font-medium">Apply changes to</span>
                  <Select
                    value={editScope}
                    onValueChange={(value) => setEditScope(value as CalendarRecurrenceEditScope)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
            ...
            <DatePicker label="Birthday" value={startDate} onChange={setStartDate} />
          )}
          {request?.kind === 'event' && (
            <label className="block space-y-1">
          ...
          <ReminderEditor values={relativeReminders} onChange={setRelativeReminders} />
        </form>
        <DialogFooter>
          <Button
            type="submit"
            ...
            disabled={
              ...
              !calendarId ||
              !startDate ||
              (isScheduled && (!endDate || (repeat === 'custom' && !customRecurrence.trim())))
            }
          >
            {saving ? 'Saving...' : request?.item ? 'Save' : 'Create'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function KanbanTaskEditorDialog({
  item,
  saving,
  onOpenChange,
  onSave,
  ...
  useEffect(() => {
    if (!item) return;
    setStartDate(
      item.start?.kind === 'date' ? item.start.date : (item.start?.dateTime.slice(0, 10) ?? ''),
    );
  ...
  return (
    <Dialog open={item != null} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Update Kanban task</DialogTitle>
        </DialogHeader>
        <form
          id="kanban-calendar-task-form"
          className="space-y-4"
          onSubmit={(event) => void submit(event)}
        >
          <div className="rounded-md bg-muted/50 px-3 py-2 text-xs text-muted-foreground">
            ...
            <br />
            Dates, completion, and supported recurrence write back to the source board. Other task
            details remain source-owned.
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1">
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              id="kanban-calendar-completed"
              checked={completed}
              onCheckedChange={(checked) => setCompleted(checked === true)}
            />
            <label htmlFor="kanban-calendar-completed" className="text-xs font-medium">
              Completed
            </label>
          </div>
          <label className="block space-y-1">
            <span className="text-xs font-medium">Repeats</span>
            <Select value={repeat} onValueChange={(value) => setRepeat(value as RecurrencePreset)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {RECURRENCE_PRESETS.filter((entry) => entry.value !== 'yearly').map((entry) => (
        ...
        <DialogFooter>
          <Button
            type="submit"
            form="kanban-calendar-task-form"
            disabled={saving || !item || (repeat === 'custom' && !customRecurrence.trim())}
          >
            {saving ? 'Saving...' : 'Save'}
          </Button>
        </DialogFooter>
    ...
    <Dialog open={calendar != null} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        ...
        <form
          ...
          onSubmit={(event) => void submit(event)}
        >
          <div className="rounded-md bg-muted/45 px-2.5 py-2 text-xs text-muted-foreground">
            <span className="font-medium text-foreground">
              {calendar ? calendarOrigin(calendar) : ''}
            </span>
            {calendar?.location.kind === 'hosted'
              ? ' calendar settings sync through its server.'
              : ' calendar stored in this profile.'}
          </div>
          <label className="block space-y-1">
          ...
          <fieldset className="space-y-2">
            <legend className="text-xs font-medium">Accent color</legend>
            <ColorPicker
              label="Calendar accent color"
              value={color}
        ...
        <DialogFooter>
          <Button
            ...
            disabled={saving || !calendar || !name.trim()}
          >
...
function CalendarImportDialog({
  ...
  return (
    <Dialog open={request != null} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Import iCalendar file</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div className="rounded-md bg-muted/45 px-3 py-2 text-xs text-muted-foreground">
            ...
            Import into{' '}
            <span className="font-medium text-foreground">{request?.calendar.name}</span>
          </div>
          <div className="grid grid-cols-2 gap-2">
            {[
          ...
          {(preview?.warnings.length ?? 0) > 0 && (
            <div className="max-h-28 overflow-y-auto rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
              {preview?.warnings.slice(0, 20).map((warning, index) => (
                <div key={`${index}-${warning}`}>{warning}</div>
              ))}
              {(preview?.warnings.length ?? 0) > 20 && (
                <div>And {(preview?.warnings.length ?? 0) - 20} more warnings.</div>
              )}
            </div>
          )}
          {(preview?.conflicts ?? 0) > 0 && (
            <p className="text-xs text-muted-foreground">
              Conflicting duplicate identities are skipped. Resolve them in the source file before
              importing.
            </p>
          )}
        </div>
        <DialogFooter>
          ...
          <Button
            type="button"
            disabled={saving || !request || changes === 0}
            onClick={() => request && void onImport(request)}
          >
            {saving ? 'Importing...' : `Import ${changes} item${changes === 1 ? '' : 's'}`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function mirrorStatusLabel(
  group: CalendarMirrorGroup,
  status: CalendarMirrorGroupStatus | undefined,
): string {
...
function CalendarMirrorDialog({
  ...
  const selectedLocationKeys = new Set(
    selectedIds.flatMap((calendarId) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Calendar mirrors</DialogTitle>
        </DialogHeader>
        <div className="space-y-5">
          <section aria-labelledby="calendar-mirror-groups">
            <h2 id="calendar-mirror-groups" className="mb-2 text-xs font-semibold">
              Mirror groups
            </h2>
                ...
                return (
                  <div key={group.id} className="flex items-center gap-3 py-2.5">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        void onUpdate(group.id, { enabled: checked === true }).catch(() => {})
                      }
                    />
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="block truncate text-[10px] text-muted-foreground">
                        {group.members
                          .map(
                            (member) =>
                              calendars.find((calendar) => calendar.id === member.calendarId)
                    ...
                    <Button
                      ...
                      onClick={() => void onDelete(group.id).catch(() => {})}
                    >
                      <Trash2 />
                    </Button>
                  </div>
          ...
          {conflicts.length > 0 && (
            <section aria-labelledby="calendar-mirror-conflicts">
              <h2 id="calendar-mirror-conflicts" className="mb-2 text-xs font-semibold">
                Conflicts
              </h2>
              <div className="divide-y divide-border/60 border-y border-border/60">
                {conflicts.map((conflict) => {
                  ...
                  return (
                    <div key={conflict.id} className="py-3">
                      <div className="mb-2">
                        <span className="block text-xs font-medium">
                          {group?.name ?? 'Mirror group'}
                        </span>
                        <span className="text-[10px] text-muted-foreground">
                          Choose the version that should replace the others.
                        </span>
                      </div>
                      <div className="grid gap-1.5">
                        {conflict.versions.map((version) => {
                          const member = group?.members.find(
                            (entry) => entry.id === version.memberId,
                          );
                          const calendar =
                            member && calendars.find((entry) => entry.id === member.calendarId);
                          return (
                            <Button
                              key={version.memberId}
                              type="button"
                              variant="outline"
                              className="h-auto min-h-9 justify-start px-2 py-1.5 text-left"
                              disabled={saving || !member}
                              onClick={() =>
                                void onResolve(conflict.id, version.memberId).catch(() => {})
                              }
                            >
                              <span
                                className="size-2.5 shrink-0 rounded-full"
                              ...
                              <span className="min-w-0 flex-1">
                                <span className="block truncate text-xs">
                                  {version.item?.deletedAt || !version.item
                                    ? 'Deleted'
                                    : version.item.title}
                                </span>
                                <span className="block truncate text-[9px] font-normal text-muted-foreground">
                                  {calendar?.name ?? 'Unavailable calendar'} ·{' '}
                                  {calendar ? calendarOrigin(calendar) : 'Unknown location'}
                                </span>
                              </span>
                            </Button>
                          );
          ...
          <form
            id="calendar-mirror-form"
            className="space-y-3"
            onSubmit={(event) => void submit(event)}
          >
            <div>
              <h2 className="text-xs font-semibold">New mirror group</h2>
              <p className="mt-0.5 text-[10px] text-muted-foreground">
                Choose one writable calendar from each local profile or connected server location.
              </p>
            </div>
            <label className="block space-y-1">
              <span className="text-xs font-medium">Name</span>
              <Input
                value={name}
                onChange={(event) => setName(event.target.value)}
                placeholder="Work calendar mirror"
                maxLength={120}
              />
            </label>
            <fieldset className="space-y-1">
              <legend className="mb-1 text-xs font-medium">Calendars</legend>
              {eligibleCalendars.map((calendar) => {
                const checked = selectedIds.includes(calendar.id);
                const locationKey = calendarMirrorLocationKey({
                  location: calendar.location as Extract<
                    CalendarLocation,
                    { kind: 'local' | 'hosted' }
                  >,
                });
                ...
                return (
                  <label
                    ...
                    htmlFor={checkboxId}
                    className={cn(
                      'flex items-center gap-2 rounded-md px-2 py-2 text-xs hover:bg-accent/50',
                      locationAlreadySelected && 'opacity-45',
                    )}
                  >
                    <Checkbox
                      id={checkboxId}
                      checked={checked}
                      disabled={locationAlreadySelected || saving}
                      onCheckedChange={(value) => toggleCalendar(calendar, value === true)}
                    />
                    <span
                      className="size-2.5 shrink-0 rounded-full"
                      style={{ backgroundColor: calendar.color }}
              ...
              {eligibleCalendars.length < 2 && (
                <p className="py-3 text-xs text-muted-foreground">
                  At least two writable calendar locations are required.
                </p>
              )}
            </fieldset>
          </form>
...
function CalendarDialog({
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    await onCreate(name.trim(), color, target?.location);
    setName('');
    onOpenChange(false);
  };
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        ...
        <form id="new-calendar-form" className="space-y-3" onSubmit={(event) => void submit(event)}>
          <label className="block space-y-1">
            <span className="text-xs font-medium">Name</span>
            <Input
              autoFocus
        ...
        <DialogFooter>
          <Button type="submit" form="new-calendar-form" disabled={saving || !name.trim()}>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        ...
        >
          ...
          <label className="block space-y-1">
            <span className="text-xs font-medium">iCalendar feed URL</span>
            <Input
              type="url"
              inputMode="url"
              value={feedUrl}
          ...
          <label className="block space-y-1">
            <span className="text-xs font-medium">Stored on</span>
            <Select value={locationKey} onValueChange={setLocationKey}>
              <SelectTrigger className="w-full">
                <SelectValue />
              </SelectTrigger>
          ...
          <p className="text-[10px] text-muted-foreground">
            Subscriptions are read-only and refresh from HTTPS feeds. Private and local network
            targets are blocked.
          </p>
          <fieldset className="space-y-2">
            <legend className="text-xs font-medium">Accent color</legend>
            <ColorPicker
              label="Subscription accent color"
              value={color}
              colors={CALENDAR_COLORS}
              onValueChange={setColor}
              className="w-full"
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={saving || !name.trim() || !feedUrl.trim()}
          >
...
function CalendarPublishDialog({
  calendar,
  onOpenChange,
}: {
  calendar: CalendarDefinition | null;
  ...
  useEffect(() => {
    ...
    setError(null);
    void tauriCommands
      .hostedCalendarRequest<CalendarPublishedFeed[]>(
  ...
  return (
    <Dialog open={calendar != null} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Publish {calendar?.name ?? 'calendar'}</DialogTitle>
        </DialogHeader>
        <p className="text-xs text-muted-foreground">
          Anyone with a published URL can read this calendar in an external calendar app. The URL is
          shown only when it is created.
        </p>
        {error && (
          <div className="rounded border border-destructive/30 bg-destructive/10 px-2.5 py-2 text-xs text-destructive">
            {error}
          </div>
        )}
        {generatedUrl && (
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              <Input
                ...
                aria-label="Published calendar URL"
              />
              <Button
                type="button"
                variant="outline"
                size="icon"
                aria-label="Copy published calendar URL"
                title="Copy URL"
                onClick={() => {
                  void navigator.clipboard
                    .writeText(generatedUrl)
                    .then(() => setCopied(true))
                    .catch(() => setError('Could not copy the published URL.'));
                }}
              >
                <Copy />
              </Button>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <Button type="button" size="sm" disabled={busy} onClick={() => void createFeed()}>
              <Plus />
              New link
            </Button>
          </div>
          <div className="max-h-56 space-y-1 overflow-y-auto">
            ...
            {!busy && feeds.length === 0 && (
              <div className="rounded border border-dashed border-border px-3 py-5 text-center text-xs text-muted-foreground">
                No published links
              </div>
            )}
            {feeds.map((feed) => (
              <div
                key={feed.id}
                className="flex items-center gap-2 rounded border border-border/70 px-2.5 py-2 text-xs"
              >
                ...
                <Button
                  ...
                  size="icon-sm"
                  disabled={busy}
                  aria-label="Revoke published calendar link"
                  title="Revoke link"
...
export function CalendarCalDavDialog({
  open,
  onOpenChange,
  origins,
}: {
  ...
  useEffect(() => {
    ...
    setServerUrl((current) =>
      origins.some((origin) => origin.serverUrl === current)
        ? current
        : (origins[0]?.serverUrl ?? ''),
    );
    setCreated(null);
  ...
  useEffect(() => {
    ...
    setError(null);
    void tauriCommands
      .hostedCalendarRequest<CalendarCalDavCredential[]>(
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>CalDAV access</DialogTitle>
        </DialogHeader>
        <p className="text-xs text-muted-foreground">
          Connect an external calendar application with a separate revocable password. Your normal
          Collab password and session are never shared with CalDAV clients.
        </p>
        {origins.length > 1 && (
          <div className="space-y-1.5">
            <span className="text-xs font-medium">Server</span>
            <Select
              value={selectedOrigin?.serverUrl ?? ''}
              onValueChange={(value) => {
                setServerUrl(value);
                setCreated(null);
                setCopied('');
            ...
            >
              <SelectTrigger aria-label="CalDAV server">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {origins.map((origin) => (
                  <SelectItem key={origin.serverUrl} value={origin.serverUrl}>
                    {origin.serverUrl}
                  </SelectItem>
                ))}
              </SelectContent>
        ...
        {created && (
          <div className="space-y-2 rounded-md border border-primary/35 bg-primary/5 p-3">
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">
                It cannot be displayed again after this dialog closes.
              </p>
            </div>
            {(
              [
              ...
              <div key={key} className="space-y-1">
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    value={value}
                    className="min-w-0 font-mono text-xs"
                    aria-label={`CalDAV ${title}`}
                  />
                  <Button
                    type="button"
                    variant="outline"
                    size="icon"
                    aria-label={`Copy CalDAV ${title}`}
        ...
        >
          <Input
            ...
            maxLength={80}
            placeholder="Device or application name"
            aria-label="CalDAV credential name"
          />
          <Button type="submit" disabled={busy || !selectedOrigin || !label.trim()}>
            <Plus />
            Create
          </Button>
        </form>
