...
import { activeWorksheet, deleteTracks, duplicateWorksheet, getCell, setCell } from './operations';
...
describe('sheet data validation', () => {
  it('assigns one reusable rule to a selection and prunes it when cleared', () => {
    ...
    expect(getCell(validated, { row: 0, column: 0 })?.validationId).toBe(
      validated.validations?.[0].id,
    );
    expect(getCell(validated, { row: 1, column: 1 })?.validationId).toBe(
      validated.validations?.[0].id,
    );
  ...
  it('validates literal lists, source ranges, dates, and text lengths', () => {
    ...
    expect(
      validateCellAgainstValidation(
        ...
        { value: '2026-07-30', valueType: 'date' },
      ).valid,
    ).toBe(true);
    expect(
      validateCellAgainstValidation(
