...
export default function SheetFilterPopover({
  ...
  const [selected, setSelected] = useState<Set<string>>(() => new Set(
    (filter?.includeValues ?? values).map(valueKey),
  ));
  ...
  const [colorKind, setColorKind] = useState<ColorKind>(
    filter?.textColors ? 'text' : 'background',
  );
  const [selectedColors, setSelectedColors] = useState<Set<string>>(() => new Set(
    filter?.textColors ?? filter?.backgroundColors ?? [],
  ));
  ...
  const shown = useMemo(() => values.filter(
    (value) => valueLabel(value).toLocaleLowerCase().includes(search.toLocaleLowerCase()),
  ), [search, values]);
  const availableColors = colorKind === 'background'
  ...
  const apply = () => {
    ...
    if (mode === 'values') {
      ...
      if (numberMin.trim() && Number.isFinite(Number(numberMin))) next.numberMin = Number(numberMin);
      if (numberMax.trim() && Number.isFinite(Number(numberMax))) next.numberMax = Number(numberMax);
    } else if (mode === 'date') {
    ...
    <Popover>
      ...
      <PopoverContent align="start" className="w-80">
        ...
        <div className="mt-3">
          {mode === 'values' && (
            <>
              ...
              <div className="my-2 flex gap-2">
                <Button type="button" size="sm" variant="outline" onClick={() => setSelected(new Set(values.map(valueKey)))}>
                  <ListChecks />
                  All
                </Button>
              ...
              <div className="max-h-44 space-y-1 overflow-y-auto overscroll-contain pr-1">
                {shown.map((value) => {
                  ...
                  return (
                    <label key={key} className="flex items-center gap-2 rounded px-1 py-1 text-xs hover:bg-muted/60">
                      <Checkbox
          ...
          {mode === 'number' && (
            <div className="grid grid-cols-2 gap-2">
              <Input type="number" value={numberMin} aria-label="Minimum number" placeholder="Minimum" onChange={(event) => setNumberMin(event.target.value)} />
              <Input type="number" value={numberMax} aria-label="Maximum number" placeholder="Maximum" onChange={(event) => setNumberMax(event.target.value)} />
            </div>
          )}
          {mode === 'date' && (
            <div className="grid grid-cols-2 gap-2">
              <DatePicker label="From" value={dateFrom} onChange={setDateFrom} />
              <DatePicker label="To" value={dateTo} min={dateFrom || undefined} onChange={setDateTo} />
            </div>
          )}
          {mode === 'color' && (
            <div className="space-y-2">
              <Select value={colorKind} onValueChange={(value) => {
              ...
              {availableColors.length === 0 && (
                <div className="text-xs text-muted-foreground">No explicit colors in this column.</div>
              )}
            </div>
          )}
        </div>
        ...
        <label className="mt-3 flex items-center gap-2 border-t border-border/60 pt-3 text-xs">
          <Checkbox checked={hideBlanks} onCheckedChange={(checked) => setHideBlanks(checked === true)} />
          Hide blank cells
        </label>
        <div className="mt-3 flex justify-end gap-2">
          <Button type="button" size="sm" variant="ghost" disabled={!active} onClick={() => onApply(null)}>
            <RotateCcw />
            Clear
          </Button>
