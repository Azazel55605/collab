...
`pnpm dlx shadcn@latest add` and keep the aliases from `components.json`. The
package is also a real build input, not just a generator: `src/App.css` imports
`shadcn/tailwind.css` from it, so it has to stay installed. Its transitive tree is
large and drags in advisories; `pnpm.overrides` pins the affected packages to
their fixed versions rather than removing the dependency.
