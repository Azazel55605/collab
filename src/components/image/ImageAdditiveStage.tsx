...
const TEXT_RESIZE_HANDLES = [
  {
    key: 'top',
    className: 'absolute inset-x-2 top-[-3px] h-2 cursor-ns-resize',
    edges: { left: false, right: false, top: true, bottom: false },
  },
  {
    key: 'bottom',
    className: 'absolute inset-x-2 bottom-[-3px] h-2 cursor-ns-resize',
    edges: { left: false, right: false, top: false, bottom: true },
  },
  {
    key: 'left',
    className: 'absolute inset-y-2 left-[-3px] w-2 cursor-ew-resize',
    edges: { left: true, right: false, top: false, bottom: false },
  },
  {
    key: 'right',
    className: 'absolute inset-y-2 right-[-3px] w-2 cursor-ew-resize',
    edges: { left: false, right: true, top: false, bottom: false },
  },
  {
    key: 'top-left',
    className: 'absolute left-[-4px] top-[-4px] h-3 w-3 cursor-nwse-resize',
    edges: { left: true, right: false, top: true, bottom: false },
  },
  {
    key: 'top-right',
    className: 'absolute right-[-4px] top-[-4px] h-3 w-3 cursor-nesw-resize',
    edges: { left: false, right: true, top: true, bottom: false },
  },
  {
    key: 'bottom-left',
    className: 'absolute bottom-[-4px] left-[-4px] h-3 w-3 cursor-nesw-resize',
    edges: { left: true, right: false, top: false, bottom: true },
  },
  {
    key: 'bottom-right',
    className: 'absolute bottom-[-4px] right-[-4px] h-3 w-3 cursor-nwse-resize',
  ...
  return (
    ...
    >
      ...
      >
            ...
            if (item.type === 'arrow') {
              ...
              return (
                ...
                >
                  ...
                  />
                  <polygon
                    points={getArrowHeadPoints(arrowStart, arrowEnd, headSize)}
                    fill={arrow.color}
                  />
              ...
              return (
                <polyline
                  ...
                  points={pen.points
                    .map(
                      (point) =>
                        `${point.x * additiveDisplayDimensions.width},${point.y * additiveDisplayDimensions.height}`,
                    )
                    .join(' ')}
                  fill="none"
                  stroke={pen.color}
        ...
        {overlaySvgItems
          .filter((item): item is ImageTextOverlay => item.type === 'text')
          .map((item) => (
            <div
              key={item.id}
              className={cn(
                ...
                selectedItemId === item.id
                  ? 'ring-1 ring-primary/80 bg-background/70'
                  : 'bg-background/35',
              )}
              style={{
                left: `${item.x * 100}%`,
              ...
              onPointerDown={(event) => {
                ...
                if (
                  ...
                  target.closest('[data-text-resize-handle="true"]')
                ) {
                  return;
                }
                const stage = event.currentTarget.closest(
