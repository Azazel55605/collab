...
a formatter would rewrite into something that no longer says what was meant. The `preformat` step rewrites `@/…` imports in `src/` to relative
paths, because the Android companion bundles modules out of `src/` without an
`@` alias. `src/components/ui/` is excluded — those files are generated by
