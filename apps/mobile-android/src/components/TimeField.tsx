...
function displayTime(value: string, format: CalendarTimeFormat): string {
  ...
  if (!prefersTwelveHourClock(format))
    ...
    <>
      ...
      {open ? (
        <div className="sheet-backdrop" onClick={() => setOpen(false)}>
          <div
            className="sheet mobile-time-sheet"
            role="dialog"
            aria-label={label}
          ...
          >
            ...
            <div className="mobile-time-fields">
              <TimeNumber
                ...
                onChange={(value) =>
                  update('hour', twelveHour ? (value % 12) + (draft.hour >= 12 ? 12 : 0) : value)
                }
              ...
              <TimeNumber
                ...
                onStep={(amount) => step('minute', amount * 5)}
              />
            </div>
            {twelveHour ? (
              <div className="mobile-time-period" role="group" aria-label="Time period">
                <button
                  type="button"
                  className={draft.hour < 12 ? 'active' : ''}
                  onClick={() => setDraft((current) => ({ ...current, hour: current.hour % 12 }))}
                >
                  AM
                </button>
                <button
                  ...
                  className={draft.hour >= 12 ? 'active' : ''}
                  onClick={() =>
                    setDraft((current) => ({ ...current, hour: (current.hour % 12) + 12 }))
                  }
                >
                  PM
                </button>
