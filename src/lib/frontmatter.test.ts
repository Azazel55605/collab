...
describe('frontmatter field helpers', () => {
  it('adds and updates frontmatter fields', () => {
    ...
    expect(setFrontmatterField(withoutFrontmatter, 'reviewStatus', 'draft')).toBe(
      '---\nreviewStatus: draft\n---\nHello note',
    );

    const withFrontmatter = '---\ntitle: Note\n---\nHello note';
    expect(setFrontmatterField(withFrontmatter, 'reviewStatus', 'draft')).toBe(
      '---\ntitle: Note\nreviewStatus: draft\n---\nHello note',
    );
