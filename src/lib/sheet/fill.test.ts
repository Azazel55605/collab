...
describe('sheet fill', () => {
  it('continues numeric series and expands the selection', () => {
    ...
    document = setCell(document, worksheet.id, { row: 0, column: 0 }, { value: 2, valueType: 'number' });
    document = setCell(document, worksheet.id, { row: 1, column: 0 }, { value: 4, valueType: 'number' });
    const selection = extendSelection(createSelection({ row: 0, column: 0 }), { row: 1, column: 0 });
  ...
  it('translates formulas and copies resolved formatting without copying notes', () => {
    ...
    document = setCell(document, worksheet.id, { row: 2, column: 0 }, {
      value: 'old',
      valueType: 'text',
    ...
    expect(resolveCellStyle(
      result.document.styles,
      activeWorksheet(result.document),
      { row: 2, column: 0 },
  ...
  it('continues ISO date series', () => {
    ...
    expect(getCell(activeWorksheet(result.document), { row: 3, column: 0 })?.value)
      .toBe('2026-07-07');
  });
});
