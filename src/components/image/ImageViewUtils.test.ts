...
describe('ImageViewUtils', () => {
  it('fits and scales dimensions safely', () => {
    expect(fitWithin({ width: 500, height: 300 }, { width: 1000, height: 1000 })).toEqual({
      width: 300,
      height: 300,
    });
  ...
  it('computes crop and preview dimensions from edits', () => {
    ...
    expect(getPermanentPreviewDimensions(source, edits, false)).toEqual({
      width: 600,
      height: 200,
    });
    expect(getPermanentPreviewDimensions(source, edits, true)).toEqual({
      width: 800,
      height: 1200,
    });
  });
  ...
  it('normalizes crop rectangles into bounds', () => {
    expect(
      normalizeCropRect({ x: -10, y: 10, width: 999, height: 0 }, { width: 300, height: 200 }),
    ).toEqual({
      x: 0,
      y: 10,
      width: 300,
  ...
  it('derives stroke dash and arrow line end geometry', () => {
    ...
    expect(getArrowLineEnd({ x: 0, y: 0 }, { x: 10, y: 0 }, 4)).toEqual({
      x: expect.closeTo(6.72, 10),
      y: 0,
    });
  });
